...
**Phase 26 is now fully IMPLEMENTED (2026-08-13), all sub-phases 26a-26e, live-verified against the real composed stack including in-browser.** BR-TP01-BR-TP14 confirmed and implemented in `demos/01-dictionary/backend/organizations-service/`; a "Trading partners" section built in the Admin UI as `TradingPartnersPanel.vue`. **Both of those names are historical** — Phase 36 moved the panel to the `refdata` app and Phase 38e renamed the service and the panel, so today they are `organizations-service` and `frontend/refdata/src/components/OrganizationsPanel.vue` (see [[phase38e_organizations_rename]]). See `BUSINESS_RULES-ORGANIZATIONS.md` for the full rule text and per-sub-phase verification notes. Remaining items are deliberately deferred, not gaps: lifecycle-as-CQRS/temporal exploration, `ComplianceDocument` temporal classification, document-expiry-driven status, real file storage, terminal/offboarding state, platform-identity vs tenant-membership split, `notify.*` publication once a marketplace consumer exists.
