...
**How to apply:** When asked to add UI groupings like "outbound vs inbound" or "delivered," don't look for or add a new domain status — derive the split client-side from `destPort` vs the current/selected port (as done in `frontend/seafreight-app/src/components/TerminalPanel.vue`'s Outbound/Arrived tables and `FleetPanel.vue`'s status filter — path was `frontend-port/...` before the 2026-07-22 rename). This has been done twice now (TerminalPanel split, Fleet panel filter) and the pattern held both times without any backend change.
