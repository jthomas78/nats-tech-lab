...
3. **Organisation** — Customer | Transporter | Operator-self | Integrator (+ future Warehouse/Broker). Lives *inside* a tenant account, separated by subject scope — see [[nats_scoped_signing_keys]].
...
**How to apply:** this is a design agreement from discussion only — nothing implemented, no plan phase covers organisations, memberships, users or region (per [[design_discussion_vs_implementation_signal]], do not start building). Record it in `ARCHITECTURE-ACCOUNTS.md` when the user is ready.
