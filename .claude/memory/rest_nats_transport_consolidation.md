...
- **Phase 34** — Enforce the boundary: per-service REST-mux admin-allowlist tests, a client-supplied (non-authoritative) `requester` header for the Admin UI's trace filter, admin subjects namespaced separately from business subjects (e.g. `api.*.refdata.admin.*` vs `api.*.refdata.item.*`) so permission grants can scope by subject prefix. PROPOSED, not started.
