...
- Multi-tenancy hard isolation (NATS Account per tenant) — **spiked in the Dictionary POC (Phase 18, 2026-07-27)**: isolation demonstrably works (server-enforced, not convention) and the concrete costs are known (per-account JetStream limits, cross-tenant shared services need exports/imports, a runtime-discovery gap for onboarding new tenants) — see `Architecture/Dictionary-POC/ARCHITECTURE.md` § "Multi-Tenancy Isolation Spike (Phase 18)". This is still a POC-scale spike, not a platform-wide ruling — the decision to adopt hard isolation platform-wide, and when, remains open.
