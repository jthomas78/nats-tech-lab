...
- Multi-tenant with isolated environments: V3 is multi-tenant SaaS; each tenant's data and environment must be isolated. **Hard isolation (one NATS account per tenant) is implemented and tested** (Phase 13/14b; DD-04a) — no longer an open item. Tenancy is enforced by the account boundary alone; the tenant name never appears in a subject, and `{context}` scopes company/business unit only.
...
- Multi-tenancy: **hard isolation — one NATS account per tenant — is the decided and implemented model** (Phase 13 static accounts, Phase 14b runtime provisioning via `accounts-service`), proven by `natsaccounts/isolation_test.go` and `accounts/provisioner_test.go`. An account is an absolute, server-enforced subject-space boundary and carries per-tenant resource limits. The tenant name never appears in a subject; `{context}` is the company/business-unit scope only (DD-04a). *Superseded: this line previously described subject-prefix "soft isolation" (`{tenant}.{region}.{bounded_context}.{event}`) as the evaluated pattern with hard isolation unevaluated — that was accurate before Phase 13 and is no longer the case.*
...
| DD-04 | **Revised 2026-07-31 — see DD-04a below; the original text is retained here for traceability.** Identifier-first NATS subject pattern: `evt.{context}.{domain}.{aggregate}.{id}.{event}` (e.g. `evt.acme-emea.shipping.ship.SHP001.arrived`) — `{context}` folds tenant+region into one token; the leading `evt` is a fixed literal, not a wildcard, so the filter can't textually overlap `$SYS.>`/`$JS.API.>` | Namespace-first (e.g. orders.customer.created); a separate `{tenant}.{region}` two-token prefix (dropped — folded into one `{context}` token once real routing was implemented) | Namespace-first suits single-tenant systems; identifier-first supports soft multi-tenant isolation now, with a path to hard isolation (separate NATS Accounts) later. A parallel `rpc.{context}.{service}.{entity}.{operation}.v1` request/reply taxonomy (Phase 12.10) mirrors the same `{context}` slot for the dual-transport RPC layer — same isolation convention, different transport, not part of this event-backbone decision |
| DD-04a | **Amends DD-04 (2026-07-31, Phase 16).** The identifier-first pattern and the fixed-literal leading token stand. What changes is the meaning of `{context}` and the isolation model. **`{context}` folds neither tenant nor region** — it is the **company / business-unit** scope only (e.g. `acme`, `acme-northdiv`; a business unit is hyphenated into the same token because every subject family has fixed arity). **Tenancy is enforced strictly and only by NATS accounts** — an absolute, server-enforced subject-space boundary with per-tenant resource limits — so the tenant name never appears in a subject. **Region is a deployment axis** (a separate regional stack with its own NATS instance) and likewise never appears in a subject. Full hierarchy: `region → tenant (NATS account) → company/group → business unit ({context})`. Values beginning `_` are reserved for platform contexts (`_platform`). | Keeping tenant and/or region in `{context}` (DD-04 as written); subject-prefix "soft isolation" within one shared account | Soft isolation is the pattern NATS's own documentation designates *legacy* and explicitly weaker: an account boundary is absolute and additionally carries independent resource limits (connections, JetStream storage, streams, consumers) that a naming convention cannot express. Encoding the tenant in `{context}` on top of an account boundary is redundant — a connection in `acme`'s account can only ever reach `acme`'s subjects regardless of token text — and actively misleading, implying isolation lives in the subject string when it does not. **Hard isolation is no longer "later": it is implemented and tested** (Phase 13 static accounts, Phase 14b runtime provisioning via `accounts-service`), proven by `natsaccounts/isolation_test.go` and `accounts/provisioner_test.go`. Subject families were formalized at the same time: Core `evt.*`/`rpc.*`/`api.*`/`notify.*`, Supportive `obs.rpc.*`/`obs.api.*`. Authority: `Architecture/Dictionary-POC/ARCHITECTURE-COMMUNICATIONS.md` § 2 |
...
- Soft vs. hard tenant isolation — soft: subject-prefix convention within one shared NATS Account (NATS documents this as the *legacy*, weaker pattern; not used for tenancy here); hard: one NATS Account per tenant with explicit cross-tenant imports/exports — **the model this platform uses** (DD-04a).
...
- Multi-tenancy hard isolation (NATS Account per tenant) — **spiked in the Dictionary POC (Phase 13, 2026-07-27)**: isolation demonstrably works (server-enforced, not convention) and the concrete costs are known (per-account JetStream limits, cross-tenant shared services need exports/imports, a runtime-discovery gap for onboarding new tenants) — see `Architecture/Dictionary-POC/ARCHITECTURE.md` § "Multi-Tenancy Isolation Spike (Phase 13)". **Resolved 2026-07-31 (DD-04a): hard isolation is the adopted platform model** — tenancy is enforced strictly and only by NATS accounts, and the tenant name never appears in a subject. What remains open is not *whether*, but the surrounding mechanics: how shared platform services (refdata/auth/accounts) obtain cross-tenant access (explicit exports/imports vs. a shared platform account — refdata currently runs on one shared account, so its per-context `tenant` link is governance metadata, not an enforced boundary), and migration sequencing for regional deployments.
