...
✅ **[Resolved via [[2. Research - Dictionary - OpenCode]]]** The subject-order tension is resolved in favor of **identifier-first / tenant-first** subjects for multi-tenant systems. Synadia's subject guide distinguishes three patterns:
...
> **Superseded 2026-07-31 (Phase 16).** The `{tenant}.{region}.{bounded_context}.{event}`
> shape recorded below, and the "many app tenants inside one NATS Account
> separated by subject prefixes" option discussed under it, were the research-stage
> conclusion. Both are superseded: **tenancy is enforced strictly by one NATS
> account per tenant** (implemented Phase 13/14b), and consequently **neither the
> tenant nor the region appears in a subject** — a subject carries only
> `{context}`, the company / business-unit scope. Authority:
> `V3-Platform/Architecture/Dictionary-POC/ARCHITECTURE-COMMUNICATIONS.md` § 2 and
> DD-04a in `V3-Platform/System Design - V3 Logistics Platform.md`. Retained below
> as a record of the research that led there.

The domain-first interpretation was a misread of the namespace-first pattern, which applies better to service-oriented or single-tenant systems. For Linebooker's multi-tenant logistics context, the subject-pattern decision at the time was: use the **Identifier-first** pattern with this format:
