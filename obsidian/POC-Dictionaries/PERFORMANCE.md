...
suite is finalised in Phase 15. See
...
> in Phase 10 — they interact with Phases 12–14 and are Phase 15 work.
...
**unusable for interactive reads without snapshotting** (Phase 15 mitigation).
...
the case for a **snapshot checkpoint** on the write path (Phase 15 mitigation).
...
> `max_connections`, and consumer parallelism tuning) are **Phase 15** — flagged
> here, not applied (measurement only). This is arguably the most actionable
> finding of the baseline.

---

## Deferred scenarios (owned by Phase 15)
...
|---|---|---|
| Optimistic-concurrency contention | retry rate + latency cost of the sequence guard | **Phase 12** (guard doesn't exist yet) |
| Cross-stream burst / consumer lag | `SHIPPING` + `TERMINAL` projection lag under write pressure | **Phase 14** (no `TERMINAL` stream yet) |
| Cross-aggregate stale-read window | staleness of the read-model guard under load | **Phase 14** (stream split) |
| SSE fan-out | concurrent watch clients before lag | **Phase 15** (streaming, not request-shaped) |

When Phase 12 and Phase 14 land, Phase 15 also **re-measures the three
baselines above** against the final architecture and records the before/after
delta — in particular, what the Phase 12 sequence guard costs on the write
