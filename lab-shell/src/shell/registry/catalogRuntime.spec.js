...
describe('BR-AS15 / decisions 90–92 — the federated catalog owns its regions', () => {
  it('renders the existing routes and a contribution inside the nested intro view', async () => {
    ...
    const plugins = new Map(shell.plugins.map((p) => [p.id, p]))
    ...
    const view = mount({ template: '<router-view />' }, { global: { plugins: [router, PrimeVue], provide: { [SHELL]: withRuntime(shell, { loader, plugins }) } } })
    await flushPromises()
...
describe('BR-AS44 — the native frame survives without any plugin', () => {
  it.each([
    ...
    await router.push('/')
    const view = mount(App, { global: { plugins: [router], provide: { [SHELL]: withRuntime(shell, { loader: { load }, plugins: new Map() }) } } })
    await flushPromises()
