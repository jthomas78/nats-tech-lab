...
T1 is the same six files with the `gateway {}` block deleted. **T4 is built
and measured** — `lab/06-arbiter3.sh`, 25 checks, both directions of region
loss (`F3`/`F4` lose AU, `F22`-`F25` lose ZA). Any `inferred` T4 row left in
`diagrams/combination-matrix.html` is stale and should be promoted to
`measured` against those checks.
...
./run-all.sh              # all nine shapes, then write ../REPORT.md + ../REPORT.html
...
| `REPORT.html` | the same findings with the nine topology diagrams — generated |
| `docs/03-multi-cluster-and-accounts-pattern-cards.html` | stage 04 — the pattern cards deck, nine cards over 22 A4 pages, one drawing per card. Editable source |
| `docs/03-multi-cluster-and-accounts-pattern-cards.pdf` | the same deck, exported. This is what gets handed to somebody |
| `docs/check-cards.sh` | the deck's shape guard — this demo has no Go suite, so the script **is** the spec |
...
Stage 04 — the pattern cards deck — is **written**, 2026-09-23. Nine cards at
two A4 pages each, plus a cover, a figure key, a selection guide and a
provenance page: twenty-two A4 pages. Read the `pattern-cards` skill before
changing it.

- **Both editions ship.** Edit the HTML, then re-export:
  `node ../01-dictionary/diagrams/export-html-pdf.mjs docs/03-multi-cluster-and-accounts-pattern-cards.html docs/03-multi-cluster-and-accounts-pattern-cards.pdf`
- **Then run the guard** — `./docs/check-cards.sh`. It checks shape only: one
  card per pattern, a pro and a con and a verdict on each, a provenance page
  with a date and a machine, the retraction card still present, a drawing per
  card with an `aria-label`, and the four report boundary classes still in the
  figure CSS. It can never check truth.
- **Every card is two pages, and page one carries its drawing.** Page one is
  title, decision and figure; page two is mechanism, pro, con and verdict. A
  card that loses its figure fails the guard.
- **The figures use `REPORT.html`'s drawing grammar, not their own.** The `.fig`
  CSS in the deck is a copy of the primitives in `lab/render-report.py`, so a
  boundary means the same thing in both documents: `nb dom` is a **JetStream
  domain** in orange `--dom`, `nb acct` is an **account** in amber `--warn`,
  `nb clu` is a cluster/region, `nb clu dead` is frozen, `gw` is a gateway, `lf`
  a leafnode link, `mir` a mirror, `dead` a cut. Boxes nest outside-in: domain,
  account, cluster, servers. **Do not invent a colour for a boundary here** —
  cards 01 to 03 are an argument about exactly those two colours meaning two
  different things. The page after the cover is the key that states it.
- **Then re-run the layout audit** —
  `node ../01-dictionary/diagrams/audit-svg-layout.mjs docs/03-multi-cluster-and-accounts-pattern-cards.html`
  must report `0 errors, 0 warnings`.
- **A page must fit.** Each `.page` is a fixed A4 with `overflow:hidden`, so
  extra content disappears silently instead of paging. After an edit, open the
  file and check every page: `document.querySelectorAll('.page')` — any element
  whose `scrollHeight` exceeds its `clientHeight` is clipped in the PDF. Step
  that page's type down with the `tight` or `prov` class rather than cutting
  content.
- **Card 09 is the retraction card and stays.** A lab whose numbers only ever
  improve is not measuring. A corrected result moves onto that card; it does
  not disappear.
...
- **A mirror cannot cross an ACCOUNT wall, and does not say so** (measured
  2026-09-22, `lab/05-export-import.sh`, `E17`/`E18`). `LB_AU` asked to mirror
  `ODOMETER`, a stream owned by `LB_ZA`. It was **not refused**. A mirror name
  is resolved inside the asking account, and `LB_AU` had an `ODOMETER` of its
  own, so the mirror copied that one: zero messages, while the ZA stream it
  appears to name held one. Export / import opens a **subject** hole, never a
  stream hole. This is the account-wall twin of the leaf-link trap below.
...
- **An arbiter cluster of three survives a whole region, both ways**
  (measured 2026-09-23, `lab/06-arbiter3.sh`, T4 / F). Nine servers, majority
  five. Lose AU and ZA still creates a 3-replica stream (`F3`/`F4`); lose ZA
  and AU does (`F22`-`F24`) — the second direction was assumed for a long time
  and is now measured. Five of nine is exactly the majority and still elects
  (`F5`). Four of nine is not: no leader, and every create fails (`F7`/`F8`),
  while a publish into a stream that already exists keeps working and really
  stores (`F9`/`F21`). Both recoveries return the group to nine
  (`F10`/`F25`).

- **A core `nats pub` proves nothing about storage.** It asks for no JetStream
  ack, so it exits 0 even when the message was never stored. Three checks once
  read only that exit code and claimed a write had succeeded. Any check that
  says a write landed must count it — `stream_msgs`, the way `A12`, `F21`,
  `D23`, `G12` and `H16` do.
...
- The hub as the **primary store for both regions** (T5). It does hold
  streams of its own — it captures a live subject (`H8`) and it serves a
  mirror (`H10`/`H11`). What is unmeasured is the hub carrying the load, and
  what its loss costs then.
- Leaf reconnect behaviour with **many** regions, not two.
- **A real WAN partition.** Every region loss in this demo is `kill -STOP` on
  that region's processes — a clean, total stop. Both sides running and unable
