...
- **A domain cannot split a supercluster.** A domain name may only change across
  ...
  over a gateway gives you a shared meta group **and** a broken link: `au` never
  elects a leader, and placement fails with `no suitable peers for placement
  (10005)`. Use the same domain everywhere, or none. This is Figure B, and it is
  the second deliberate reproduction of a demo 02 finding.
