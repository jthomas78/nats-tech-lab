...
./run-all.sh              # all five topologies, then write ../REPORT.md + ../REPORT.html
...
`figures.html` is the one hand-drawn file in the lab. It holds five SVG
fragments, keyed `t1` / `a` / `b` / `c` / `d` by `<!--FIG key-->` markers, in
...
| `REPORT.html` | the same findings with the five topology diagrams — generated |
