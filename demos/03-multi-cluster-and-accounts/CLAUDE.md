...
| `docs/03-multi-cluster-and-accounts-pattern-cards.html` | stage 04 — the pattern cards deck, nine cards. Editable source |
...
Stage 04 — the pattern cards deck — is **written**, 2026-09-23. Nine cards,
plus a selection guide and a provenance page: twelve A4 pages. Read the
`pattern-cards` skill before changing it.
...
- **Then run the guard** — `./docs/check-cards.sh`. It checks shape only: one
  ...
  with a date and a machine, the retraction card still present, and an
  `aria-label` on every figure. It can never check truth.
