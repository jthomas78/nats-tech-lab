...
// stream count in the response, so stream count = len(stream_detail).
...
type jszAccount struct {
	ID            string            `json:"id"`
	StreamDetails []json.RawMessage `json:"stream_detail"` // one element per stream; count = len
...
func (f *UsageFetcher) FetchAll(ctx context.Context) ([]JSUsage, error) {
	...
	for _, a := range accs {
		stats := live[a.PublicKey]
		out = append(out, JSUsage{
			Name:      a.Name,
			Streams:   JSCounter{Used: int64(len(stats.StreamDetails)), Limit: a.JSMaxStreams},
			Consumers: JSCounter{Limit: a.JSMaxConsumers}, // consumer count not available without ?consumers=true
