...
// into a running mfe-registry-service over NATS request/reply.
//
...
// its api.* subjects; this is not a service-to-service rpc.* caller. The
// credential still comes from accounts-service — it owns the trust chain —
// while the subjects it calls are served by mfe-registry-service, which is
// why -url still names accounts-service after the split. A boot-time file
...
func (c *client) revision() (int64, error) {
	out, err := c.call(mferegistry.Curated, []byte(`{}`))
...
func (c *client) upsert(entry json.RawMessage, revision int64) error {
	...
	_, err = c.call(mferegistry.Upsert, payload)
	return err
}
