...
func startRegistryPostgres() (*sql.DB, string, error) {
	// A caller-supplied disposable test database takes precedence. Otherwise
	// retain the existing Docker fixture so a normal local run skips nothing.
	if dsn := os.Getenv("MFE_REGISTRY_TEST_DATABASE_URL"); dsn != "" {
