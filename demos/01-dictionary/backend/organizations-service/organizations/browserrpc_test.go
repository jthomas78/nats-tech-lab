...
// only) for the micro-service registration that makes organizations-service
	...
	Context("once the adapter is registered", func() {
		...
		It("advertises all 24 api.* endpoints on $SRV.INFO", func() {
			...
			Expect(subjects).To(ConsistOf(
				"api.*.organizations.organization.register.v1",
				"api.*.organizations.organization.submit-vetting.v1",
				"api.*.organizations.organization.list.v1",
				"api.*.organizations.organization.get.v1",
				"api.*.organizations.organization.update.v1",
				"api.*.organizations.organization.activate.v1",
				"api.*.organizations.organization.suspend.v1",
				"api.*.organizations.organization.reactivate.v1",
				"api.*.organizations.organization.audit.v1",
				...
				"api.*.organizations.operating-area.add.v1",
				"api.*.organizations.operating-area.list.v1",
				"api.*.organizations.operating-area.remove.v1",
				...
				// has been broken.
				"api.*.organizations.tracking-credential.configure.v1",
				"api.*.organizations.tracking-credential.list.v1",
			))
		})
		...
		It("keeps every subject at 6 tokens so {context} is readable by position", func() {
			...
			for _, ep := range info.Endpoints {
				...
				Expect(parts[2]).To(Equal("organizations"))
