...
var l10nSeed = []seedItem{
	...
	// Phase 25g — Sea Freight Flow "Pricing" tab (pricing-service, BR-P16).
	{"nav.pricing", "Pricing", "Precios", "Pryse"},
	{"app.subtitlePricing", "fee scales · rate sheets · fixed rates", "escalas de tarifas · hojas de tarifas · tarifas fijas", "feeskale · tariefblaaie · vaste tariewe"},
	{"pricing.title", "Pricing", "Precios", "Pryse"},
	{"pricing.loading", "Loading pricing…", "Cargando precios…", "Pryse laai…"},
	{"pricing.feeScales", "Fee Scales", "Escalas de tarifas", "Feeskale"},
	{"pricing.rateSheets", "Rate Sheets", "Hojas de tarifas", "Tariefblaaie"},
	{"pricing.fixedRates", "Fixed Rates", "Tarifas fijas", "Vaste tariewe"},
	{"pricing.feeScalesEmpty", "No fee scales registered for this context yet.", "Aún no hay escalas de tarifas registradas para este contexto.", "Nog geen feeskale vir hierdie konteks geregistreer nie."},
	{"pricing.rateSheetsEmpty", "No rate sheets registered for this context yet.", "Aún no hay hojas de tarifas registradas para este contexto.", "Nog geen tariefblaaie vir hierdie konteks geregistreer nie."},
	{"pricing.fixedRatesEmpty", "No fixed rates registered for this context yet.", "Aún no hay tarifas fijas registradas para este contexto.", "Nog geen vaste tariewe vir hierdie konteks geregistreer nie."},
	{"pricing.typeNormal", "Normal", "Normal", "Normaal"},
	{"pricing.typeFixedRate", "Fixed Rate", "Tarifa fija", "Vaste Tarief"},
	{"table.customerKey", "Customer", "Cliente", "Klant"},
	{"table.routeKey", "Route", "Ruta", "Roete"},
	{"table.type", "Type", "Tipo", "Tipe"},
	{"status.active", "Active", "Activo", "Aktief"},
	{"status.inactive", "Inactive", "Inactivo", "Onaktief"},

	// Phase 25h — Pricing manual-entry UX (register/draft/add-range-or-entry/publish/rollback).
	{"pricing.registerFeeScale", "Register Fee Scale", "Registrar escala de tarifas", "Registreer feeskaal"},
	{"pricing.namePlaceholder", "name, e.g. standard", "nombre, p. ej. estándar", "naam, bv. standaard"},
	{"pricing.loadingDetail", "Loading…", "Cargando…", "Laai…"},
	{"pricing.activeVersion", "Active Version", "Versión activa", "Aktiewe weergawe"},
	{"pricing.rangeCount", "{count} range | {count} ranges", "{count} rango | {count} rangos", "{count} reeks | {count} reekse"},
	{"pricing.noActiveVersion", "No active version yet.", "Aún no hay una versión activa.", "Nog geen aktiewe weergawe nie."},
	{"pricing.rangeLower", "Lower ($)", "Inferior ($)", "Onder ($)"},
	{"pricing.rangeUpper", "Upper ($)", "Superior ($)", "Bo ($)"},
	{"pricing.rangeCharge", "Charge", "Cargo", "Heffing"},
	{"pricing.draft", "Draft", "Borrador", "Konsep"},
	{"pricing.noDraft", "No draft", "Sin borrador", "Geen konsep nie"},
	{"pricing.createDraft", "Create Draft", "Crear borrador", "Skep konsep"},
	{"pricing.publishDraft", "Publish Draft", "Publicar borrador", "Publiseer konsep"},
	{"pricing.rangeStartsAt", "This range starts at ${value}", "Este rango comienza en ${value}", "Hierdie reeks begin by ${value}"},
	{"pricing.percentPlaceholder", "%", "%", "%"},
	{"pricing.addRange", "Add Range", "Añadir rango", "Voeg reeks by"},
	{"pricing.rangeUpperMustExceedLower", "Upper limit must exceed the previous range's upper limit.", "El límite superior debe exceder el límite superior del rango anterior.", "Boonste limiet moet die vorige reeks se boonste limiet oorskry."},
	{"pricing.versionHistory", "Version History", "Historial de versiones", "Weergawegeskiedenis"},
	{"pricing.noVersions", "No versions yet.", "Aún no hay versiones.", "Nog geen weergawes nie."},
	{"pricing.version", "Version", "Versión", "Weergawe"},
	{"pricing.rollbackTo", "Roll back to this version", "Revertir a esta versión", "Rol terug na hierdie weergawe"},
	{"pricing.registerRateSheet", "Register Rate Sheet", "Registrar hoja de tarifas", "Registreer tariefblad"},
	{"pricing.customerKeyPlaceholder", "customer key, e.g. cust-001", "clave de cliente, p. ej. cust-001", "klantsleutel, bv. cust-001"},
	{"pricing.deactivate", "Deactivate", "Desactivar", "Deaktiveer"},
	{"pricing.activate", "Activate", "Activar", "Aktiveer"},
	{"pricing.entryCount", "{count} entry | {count} entries", "{count} entrada | {count} entradas", "{count} inskrywing | {count} inskrywings"},
	{"pricing.feeScaleOverride", "Fee Scale Override", "Anulación de escala de tarifas", "Feeskaal-oorskrywing"},
	{"pricing.vehicleType", "Vehicle Type", "Tipo de vehículo", "Voertuigtipe"},
	{"pricing.baseRate", "Base Rate ($)", "Tarifa base ($)", "Basiskoers ($)"},
	{"pricing.dropPointCount", "Drop Points", "Puntos de entrega", "Aflaaipunte"},
	{"pricing.additionalDropRate", "Additional Drop Rate ($)", "Tarifa adicional por entrega ($)", "Addisionele aflaaikoers ($)"},
	{"pricing.routeAndVehicleRequired", "Route and vehicle type are required.", "La ruta y el tipo de vehículo son obligatorios.", "Roete en voertuigtipe word vereis."},
	{"pricing.routeKeyPlaceholder", "route key, e.g. la-nyc", "clave de ruta, p. ej. la-nyc", "roetesleutel, bv. la-nyc"},
	{"pricing.vehicleTypePlaceholder", "vehicle type, e.g. reefer-40ft", "tipo de vehículo, p. ej. reefer-40ft", "voertuigtipe, bv. reefer-40ft"},
	{"pricing.addEntry", "Add Entry", "Añadir entrada", "Voeg inskrywing by"},
	{"pricing.feeScaleOverridePlaceholder", "fee scale name (optional)", "nombre de escala de tarifas (opcional)", "feeskaalnaam (opsioneel)"},
	{"pricing.setOverride", "Set Override", "Establecer anulación", "Stel oorskrywing"},
	{"pricing.registerFixedRate", "Register Fixed Rate", "Registrar tarifa fija", "Registreer vaste tarief"},
	{"pricing.pointCountValue", "{count} point | {count} points", "{count} punto | {count} puntos", "{count} punt | {count} punte"},
	{"pricing.drop", "drop", "entrega", "aflaai"},
	{"pricing.rate", "Rate ($)", "Tarifa ($)", "Koers ($)"},
	{"pricing.pointCount", "Point Count", "Cantidad de puntos", "Puntetelling"},

	// Diesel price index + overlay (BR-P17–BR-P23, Phase 25i)
	{"pricing.dieselPrices", "Diesel Prices", "Precios del diésel", "Dieselpryse"},
	{"pricing.dieselPricesEmpty", "No diesel prices indexed for this context yet.", "Aún no hay precios del diésel indexados para este contexto.", "Nog geen dieselpryse vir hierdie konteks geïndekseer nie."},
	{"pricing.indexDieselPrice", "Index Diesel Price", "Indexar precio del diésel", "Indekseer dieselprys"},
	{"pricing.activeDate", "Active Date", "Fecha activa", "Aktiewe datum"},
	{"pricing.coastalPrice", "Coastal Price ($)", "Precio costero ($)", "Kusprys ($)"},
	{"pricing.inlandPrice", "Inland Price ($)", "Precio interior ($)", "Binnelandse prys ($)"},
	{"pricing.dieselOverlays", "Diesel Overlays", "Superposiciones de diésel", "Dieseloorlegs"},
	{"pricing.noOverlays", "No diesel overlays applied yet.", "Aún no se han aplicado superposiciones de diésel.", "Nog geen dieseloorlegs toegepas nie."},
	{"pricing.minorVersion", "Minor", "Menor", "Klein"},
	{"pricing.overlayStart", "Start Date", "Fecha de inicio", "Begindatum"},
	{"pricing.overlayEnd", "End Date", "Fecha de fin", "Einddatum"},
	{"pricing.overlayOpenEnded", "open-ended", "abierto", "oopeindig"},
	{"pricing.adjustedRate", "Adjusted Rate ($)", "Tarifa ajustada ($)", "Aangepaste koers ($)"},
	{"pricing.overlayDatePlaceholder", "load date", "fecha de carga", "laaidatum"},
	{"pricing.applyOverlay", "Apply Diesel Overlay", "Aplicar superposición de diésel", "Pas dieseloorlegging toe"},
}
