...
var _ = Describe("Dictionary Item Domain Rules", func() {
	...
	Context("BR-D01: item codes are unique per {type, context}", func() {
		It("registers a new item", func() {
			item, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "EUR", Context: "acme-test"})
		...
		It("returns ErrDuplicateItemCode for a second registration of the same code in the same context", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "EUR", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
	...
	Context("BR-D02: an unreferenced item may be hard-deleted; a referenced item must be deprecated instead", func() {
		BeforeEach(func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "ZAR", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
		})

		It("hard-deletes an unreferenced item", func() {
			Expect(h.DeleteItem(ctx, "currency", "acme-test", "ZAR")).To(Succeed())
			_, err := h.Get(ctx, "currency", "acme-test", "ZAR")
		...
		It("returns ErrItemReferenced when the item is referenced by another item", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "country", Code: "ZA", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
			...
			err = h.DeleteItem(ctx, "currency", "acme-test", "ZAR")
			Expect(errors.Is(err, domain.ErrItemReferenced)).To(BeTrue())
		})

		It("deprecates a referenced item instead of deleting it", func() {
			Expect(h.DeprecateItem(ctx, "currency", "acme-test", "ZAR")).To(Succeed())
			item, err := h.Get(ctx, "currency", "acme-test", "ZAR")
			Expect(err).NotTo(HaveOccurred())
	...
	Context("BR-D12: a deprecated item can be reactivated back to active", func() {
		BeforeEach(func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "JPY", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
			Expect(h.DeprecateItem(ctx, "currency", "acme-test", "JPY")).To(Succeed())
		})

		It("flips a deprecated item back to active", func() {
			Expect(h.ReactivateItem(ctx, "currency", "acme-test", "JPY")).To(Succeed())
			item, err := h.Get(ctx, "currency", "acme-test", "JPY")
			Expect(err).NotTo(HaveOccurred())
			Expect(item.Status).To(Equal(domain.StatusActive))
		})
		...
		It("reappears in ListAssignable once reactivated", func() {
			Expect(h.ReactivateItem(ctx, "currency", "acme-test", "JPY")).To(Succeed())
			assignable, err := h.ListAssignable(ctx, "currency", "acme-test")
			Expect(err).NotTo(HaveOccurred())
		...
		It("returns ErrItemNotFound when reactivating an item that doesn't exist", func() {
			err := h.ReactivateItem(ctx, "currency", "acme-test", "does-not-exist")
			Expect(errors.Is(err, domain.ErrItemNotFound)).To(BeTrue())
		})
	})
	...
	Context("BR-D18: an item's attrs can be replaced after creation", func() {
		BeforeEach(func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{
				TypeKey: "l10n", Code: "app.title", Context: "acme-test",
		...
		It("replaces the entire attrs map, not a per-key merge", func() {
			Expect(h.UpdateItemAttrs(ctx, "l10n", "acme-test", "app.title",
				map[string]any{"name": "SeaFreight Flow"})).To(Succeed())

			item, err := h.Get(ctx, "l10n", "acme-test", "app.title")
			Expect(err).NotTo(HaveOccurred())
		...
		It("returns ErrItemNotFound when updating attrs of an item that doesn't exist", func() {
			err := h.UpdateItemAttrs(ctx, "l10n", "acme-test", "does-not-exist", map[string]any{"name": "x"})
			Expect(errors.Is(err, domain.ErrItemNotFound)).To(BeTrue())
		})

		It("works on a deprecated item, mirroring BR-D06's read-regardless-of-status stance", func() {
			Expect(h.DeprecateItem(ctx, "l10n", "acme-test", "app.title")).To(Succeed())
	...
	Context("BR-D06: deprecated items still resolve on read but are excluded from assignable-value listings by default", func() {
		BeforeEach(func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "GBP", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
			_, err = h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "EUR", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
			Expect(h.DeprecateItem(ctx, "currency", "acme-test", "GBP")).To(Succeed())
		})

		It("still resolves a deprecated item on direct Get", func() {
			item, err := h.Get(ctx, "currency", "acme-test", "GBP")
			Expect(err).NotTo(HaveOccurred())
			Expect(item.Status).To(Equal(domain.StatusDeprecated))
		})
	...
	Context("BR-D22: code must be a valid KV-key token", func() {
		It("registers a valid code", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "CHF", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
		})

		It("rejects an empty code", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "", Context: "acme-test"})
			Expect(errors.Is(err, domain.ErrInvalidKVKeyComponent)).To(BeTrue())
		})

		It("rejects a code containing ':'", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "currency", Code: "CH:F", Context: "acme-test"})
			Expect(errors.Is(err, domain.ErrInvalidKVKeyComponent)).To(BeTrue())
		})
		...
		It("accepts a code containing '.' — KV-key charset, not a subject-token charset", func() {
			_, err := h.RegisterItem(ctx, commands.ItemInput{TypeKey: "l10n", Code: "filter.none", Context: "acme-test"})
			Expect(err).NotTo(HaveOccurred())
		})
	})
})
