// Package natstrace is refdata-service's Phase 28b copy of trading-partner-
// service's Phase 28a natstrace prototype — no go.opentelemetry.io/*
// dependency, W3C-compatible on the wire, OTLP-shaped in its fields. It is
// deliberately duplicated per service (not a shared module), the same way
// obsEnvelope/errorResponse/obsSubjectFor/contextFromSubject already are in
// this codebase — see ARCHITECTURE-COMMUNICATIONS.md § 6 for why.
//
// The pattern: a Tracer (constructed once per Adapter, never a package
// singleton) wraps every micro.HandlerFunc at its single svc.AddEndpoint call
// site. Unlike shipping/pricing/trading-partner-service, refdata-service's
// Adapter is not one per tenant NATS connection — it runs on a single
// process-wide connection (Phase 21), so there is exactly one Tracer for the
// whole service rather than one per tenant. That difference doesn't change
// how Tracer.Start/Middleware/SpanFrom work, only how many Adapter instances
// construct one. The wrap starts a span from the inbound request (continuing
// an incoming traceparent header if present, otherwise minting a root span)
// and hands the handler a wrapped micro.Request carrying that span. The
// handler's existing respond/respondOK/respondError tail retrieves it via
...
// PLATFORM account only, per BR-036/BR-D39.
package natstrace
