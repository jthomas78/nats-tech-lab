...
// Contract tests for the Phase 28b natstrace copy (BR-036/BR-037). These
// exercise the package end to end over a real embedded NATS server and the
// actual nats.go/micro machinery — the same integration style
// browserrpc_test.go uses — rather than calling unexported helpers directly,
...
var _ = Describe("natstrace (Phase 28b copy — BR-036/BR-037)", func() {
	var nc *nats.Conn
	...
	// shape this package is used in inside the adapter.
	...
	Context("BR-036 — traceSpan is a strict superset of obsEnvelope", func() {
		It("publishes one span per call, decodable as both the old envelope shape and the new one", func() {
			registerEcho("api.acme.shipping.thing.action.v1")
			...
			_, err = nc.Request("api.acme.shipping.thing.action.v1", []byte(`{"ok":true}`), 2*time.Second)
			Expect(err).NotTo(HaveOccurred())

			var msg *nats.Msg
			Eventually(spans).Should(Receive(&msg))
			Expect(msg.Subject).To(Equal("obs.trace.acme.shipping.thing.action"))
			...
			Expect(legacy.Subject).To(Equal("api.acme.shipping.thing.action.v1"))
		...
		It("records the span's own measured duration (Phase 28g), not derivable from Timestamp alone", func() {
			...
			Expect(svc.AddEndpoint("echo", tracer.Middleware(handler), micro.WithEndpointSubject("api.acme.shipping.thing.slow.v1"))).To(Succeed())
			...
			_, err = nc.Request("api.acme.shipping.thing.slow.v1", []byte(`{"ok":true}`), 2*time.Second)
			Expect(err).NotTo(HaveOccurred())
		...
		It("marks a failed call with statusCode ERROR and the error message, never blocking the reply", func() {
			...
			reply, err := nc.Request("api.acme.shipping.thing.action.v1", []byte(`{"fail":true}`), 2*time.Second)
		...
		It("redacts a denylisted field before applying the 4 KiB cap, and flags truncation with the pre-truncation length", func() {
			...
			_, err = nc.Request("api.acme.shipping.thing.action.v1", body, 2*time.Second)
			Expect(err).NotTo(HaveOccurred())
		...
		It("captures the request payload independently of the reply payload, each with its own redaction/truncation (Phase 28h)", func() {
			...
			Expect(svc.AddEndpoint("echo", tracer.Middleware(handler), micro.WithEndpointSubject("api.acme.shipping.thing.action.v1"))).To(Succeed())
	...
	Context("BR-037 — trace context propagates and continues a parent span", func() {
		It("continues an inbound traceparent header as a child span rather than minting a new root", func() {
			...
			msg := &nats.Msg{
				Subject: "api.acme.shipping.thing.action.v1",
				Reply:   nats.NewInbox(),
				Data:    []byte(`{"ok":true}`),
