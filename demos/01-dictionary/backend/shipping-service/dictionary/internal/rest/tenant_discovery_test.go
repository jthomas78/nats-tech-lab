...
// discoverTenants' own exclusion of DEFAULT/SYS creds files must not depend
...
func TestDiscoverTenantsExcludesReservedNamesCaseInsensitively(t *testing.T) {
	dir := t.TempDir()
	for _, stem := range []string{"default", "DEFAULT", "Default", "sys", "SYS", "acme", "GLOBEX"} {
	...
	for _, reserved := range []string{"default", "DEFAULT", "Default", "sys", "SYS"} {
