...
// Pending tests for Phase 47b (BUSINESS_RULES-SHIPPING.md's BR-047):
...
// green until Phase 47b lands. Whether ingestion needs a KV bucket (like
...
func TestPubsubStreamProvisionedWithBoundedRetention(t *testing.T) {
	t.Skip("pending Phase 47b implementation — see BR-047: a sibling stream " +
...
func TestPubsubEnvelopeBecomesVisibleInMessagesFeed(t *testing.T) {
	t.Skip("pending Phase 47b implementation — see BR-047: every published " +
...
func TestPubsubRedeliveryIsDeduplicatedByMessageID(t *testing.T) {
	t.Skip("pending Phase 47b implementation — see BR-047: an at-least-once " +
