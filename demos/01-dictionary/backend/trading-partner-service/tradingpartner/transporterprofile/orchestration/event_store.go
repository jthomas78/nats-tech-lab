...
func (s *JetStreamEventStore) AppendWorkflowEvent(ctx context.Context, event profiledomain.Event, messageID string) error {
	_, expected, err := s.Hydrate(ctx, event.Context, event.TradingPartnerID)
	if err != nil {
		return err
	}
	_, err = s.append(ctx, event.Context, event.TradingPartnerID, event, expected, messageID)
	return err
}
...
func (s *JetStreamEventStore) Hydrate(ctx context.Context, contextKey, tradingPartnerID string) (*profiledomain.TransporterProfile, uint64, error) {
	consumer, err := s.js.OrderedConsumer(ctx, profiledomain.StreamName, jetstream.OrderedConsumerConfig{
		FilterSubjects: []string{profiledomain.InstanceSubject(contextKey, tradingPartnerID)},
...
func (s *JetStreamEventStore) Append(ctx context.Context, contextKey, tradingPartnerID string, event profiledomain.Event, expectedSequence uint64) (uint64, error) {
	return s.append(ctx, contextKey, tradingPartnerID, event, expectedSequence, "")
}

func (s *JetStreamEventStore) append(ctx context.Context, contextKey, tradingPartnerID string, event profiledomain.Event, expectedSequence uint64, messageID string) (uint64, error) {
	data, err := json.Marshal(event)
	if err != nil {
		return 0, fmt.Errorf("marshal transporter profile event: %w", err)
	}
	msg := &nats.Msg{
		Subject: profiledomain.Subject(contextKey, tradingPartnerID, event.Type),
		Data:    data,
		Header: nats.Header{
			jetstream.ExpectedLastSubjSeqHeader:     []string{strconv.FormatUint(expectedSequence, 10)},
			jetstream.ExpectedLastSubjSeqSubjHeader: []string{profiledomain.InstanceSubject(contextKey, tradingPartnerID)},
		},
	}
	if messageID != "" {
