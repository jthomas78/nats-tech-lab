...
// trading-partner-service's lifecycle actions (BR-TP06) — no UPDATE, no
...
func (a *AuditLog) Record(ctx context.Context, entry domain.AuditEntry) error {
	...
	_, err = a.db.ExecContext(ctx, `
		INSERT INTO trading_partner.audit_events (trading_partner_id, action, actor, source_ip, outcome, metadata)
		VALUES ($1, $2, $3, $4, $5, $6)`,
		entry.TradingPartnerID, entry.Action, entry.Actor, entry.SourceIP, outcome, metadataJSON)
	return err
}
...
func (a *AuditLog) ListByPartner(ctx context.Context, partnerID string) ([]domain.AuditEntry, error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT id, trading_partner_id, action, actor, source_ip, outcome, metadata, created_at
		FROM trading_partner.audit_events WHERE trading_partner_id = $1 ORDER BY created_at DESC`, partnerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.TradingPartnerID, &e.Action, &e.Actor, &e.SourceIP, &e.Outcome, &metadataJSON, &e.CreatedAt); err != nil {
			return nil, err
		}
