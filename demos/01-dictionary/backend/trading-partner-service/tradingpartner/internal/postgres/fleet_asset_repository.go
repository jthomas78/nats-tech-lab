...
func (r *FleetAssetRepository) AddFleetAsset(ctx context.Context, partnerID string, asset domain.FleetAsset) (domain.FleetAsset, error) {
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO trading_partner.fleet_assets
			(registration_no, trading_partner_id, vin, make, model, vehicle_type_code)
		VALUES ($1, $2, $3, $4, $5, $6)`,
...
func (r *FleetAssetRepository) ListFleetAssets(ctx context.Context, partnerID string) ([]domain.FleetAsset, error) {
	rows, err := r.db.QueryContext(ctx, `
		...
		FROM trading_partner.fleet_assets WHERE trading_partner_id = $1 ORDER BY registration_no`, partnerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
