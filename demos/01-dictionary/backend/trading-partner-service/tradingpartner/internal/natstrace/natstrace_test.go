...
// Contract tests for the Phase 28a natstrace prototype (BR-036/BR-037/BR-TP15).
...
var _ = Describe("natstrace (Phase 28a prototype — BR-036/BR-037/BR-TP15)", func() {
	var nc *nats.Conn
	...
	Context("BR-036 — traceSpan is a strict superset of obsEnvelope", func() {
		It("publishes one span per call, decodable as both the old envelope shape and the new one", func() {
			registerEcho("api.acme.trading-partner.thing.action.v1")
			...
			_, err = nc.Request("api.acme.trading-partner.thing.action.v1", []byte(`{"ok":true}`), 2*time.Second)
			Expect(err).NotTo(HaveOccurred())

			var msg *nats.Msg
			Eventually(spans).Should(Receive(&msg))
			Expect(msg.Subject).To(Equal("obs.trace.acme.trading-partner.thing.action"))
			...
			Expect(legacy.Subject).To(Equal("api.acme.trading-partner.thing.action.v1"))
		...
		It("records the span's own measured duration (Phase 28g), not derivable from Timestamp alone", func() {
			...
			Expect(svc.AddEndpoint("echo", tracer.Middleware(handler), micro.WithEndpointSubject("api.acme.trading-partner.thing.slow.v1"))).To(Succeed())
			...
			_, err = nc.Request("api.acme.trading-partner.thing.slow.v1", []byte(`{"ok":true}`), 2*time.Second)
			Expect(err).NotTo(HaveOccurred())
		...
		It("marks a failed call with statusCode ERROR and the error message, never blocking the reply", func() {
			...
			reply, err := nc.Request("api.acme.trading-partner.thing.action.v1", []byte(`{"fail":true}`), 2*time.Second)
		...
		It("redacts a denylisted field before applying the 4 KiB cap, and flags truncation with the pre-truncation length", func() {
			...
			_, err = nc.Request("api.acme.trading-partner.thing.action.v1", body, 2*time.Second)
			Expect(err).NotTo(HaveOccurred())
		...
		It("captures the request payload independently of the reply payload, each with its own redaction/truncation (Phase 28h)", func() {
			...
			Expect(svc.AddEndpoint("echo", tracer.Middleware(handler), micro.WithEndpointSubject("api.acme.trading-partner.thing.action.v1"))).To(Succeed())
	...
	Context("BR-037 — trace context propagates and continues a parent span", func() {
		It("continues an inbound traceparent header as a child span rather than minting a new root", func() {
			...
			msg := &nats.Msg{
				Subject: "api.acme.trading-partner.thing.action.v1",
				Reply:   nats.NewInbox(),
				Data:    []byte(`{"ok":true}`),
	...
	Context("Phase 28h — every span records who requested it, not just who answered", func() {
		...
		It("leaves span.Requester empty when no Nats-Requestor header is present anywhere (BR-041)", func() {
			spans := make(chan *nats.Msg, 4)
			sub, err := nc.Subscribe("obs.trace.>", func(m *nats.Msg) { spans <- m })
			Expect(err).NotTo(HaveOccurred())
			DeferCleanup(func() { _ = sub.Unsubscribe() })
