...
suite is finalised in Phase 16. See
...
> in Phase 10 — they interact with Phases 13–15 and are Phase 16 work.
...
**unusable for interactive reads without snapshotting** (Phase 16 mitigation).
...
the case for a **snapshot checkpoint** on the write path (Phase 16 mitigation).
...
> `max_connections`, and consumer parallelism tuning) are **Phase 16** — flagged
> here, not applied (measurement only). This is arguably the most actionable
> finding of the baseline.

---

## Deferred scenarios (owned by Phase 16)
...
|---|---|---|
| Optimistic-concurrency contention | retry rate + latency cost of the sequence guard | **Phase 13** (guard doesn't exist yet) |
| Cross-stream burst / consumer lag | `SHIPPING` + `TERMINAL` projection lag under write pressure | **Phase 15** (no `TERMINAL` stream yet) |
| Cross-aggregate stale-read window | staleness of the read-model guard under load | **Phase 15** (stream split) |
| SSE fan-out | concurrent watch clients before lag | **Phase 16** (streaming, not request-shaped) |

When Phase 13 and Phase 15 land, Phase 16 also **re-measures the three
baselines above** against the final architecture and records the before/after
delta — in particular, what the Phase 13 sequence guard costs on the write
