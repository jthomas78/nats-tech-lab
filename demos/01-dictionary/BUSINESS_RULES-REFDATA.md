...
> persistent browser-instance `Nats-Requestor` its `api.*` calls do (`src/requestorId.js`). See
...
`internal/tenants.Manager` (new package) copies `pricing-service`'s `internal/tenants` pattern verbatim rather than inventing a new one: `Discover(credsDir)` re-scans `NATS_CREDS_DIR` for `*.creds` on every call, excluding `nonTenantCredsFiles` (`platform`, `shipping-admin`, `sys`, `observability`) — the same exclusion list `pricing-service` and `organizations-service` each carry, included from this rule's first commit rather than left to be independently rediscovered a fourth time. `EnsureAll` connects every tenant visible at Startup; `EnsureByName`/`TeardownByName` react to `notify.accounts.account.{created,reactivated}`/`{suspended}`, subscribed per-tenant-connection (mirroring `pricing-service`'s `subscribeLifecycle`) rather than via a separate PLATFORM-only creds file. Every per-tenant connection sets `nats.Name("refdata-service")` — identical to the PLATFORM connection's name, since both are the same service; a caller distinguishes them by which account answered, not by name.

**This was knowingly the fourth copy of this connection-manager shape** (after `pricing-service`, `organizations-service`, and — per `organizations-service`'s own doc comment — a documented divergence between those two already, not a clean two-copy precedent). Extraction into a shared `natstenants` package was blocked on this repo's lack of a `go.work` across its 7 Go modules; this phase pasted the fourth copy rather than blocking on that unrelated migration, and recorded that the duplication was a known, deliberate deferral, not an oversight.

**Phase 35 amendment: the deferred extraction landed.** `refdata/internal/tenants` (the copy described above) was deleted outright and replaced with `natstenants.Manager[*browserrpc.Adapter]` from the new `shared/natstenants` package, consumed identically by `pricing-service`, `organizations-service`, and `shipping-service` (connection-lifecycle portion only, per `ARCHITECTURE-ACCOUNTS.md`'s per-tenant-connections section). The `go.work` prerequisite this rule cited as blocking was itself the first deliverable of Phase 35 — a repo-root `go.work` plus an explicit `replace` directive in each service's `go.mod` (belt-and-suspenders, since `go mod tidy`/Docker builds don't reliably honor `go.work`'s implicit override), and each affected Dockerfile's build context moved to the repo root so it can `COPY shared/natstenants` ahead of the service's own source. `refdata-service`'s `PublishToAll` fan-out (BR-D42, previously a direct method on the old `tenants.Manager`) is now a small `tenantPublisher` wrapper in `composition.go` built on `Manager.Range()`. Behavior is unchanged: `EnsureAll`/`EnsureByName`/`TeardownByName`/the `notify.accounts.account.*` lifecycle subscription and `nonTenantCredsFiles` exclusion list all still exist, just as generic `Manager[R]` methods shared across four services instead of four independently-maintained copies.
...
- **Enforced in:** `shared/natstenants/tenants.go` (`Manager[R]`, `Discover`, `nonTenantCredsFiles`, `EnsureAll`/`Update`/`Range`/`SubscribeLifecycle`/`Close`); `refdata/composition.go` (provision/deprovision callbacks, `tenantPublisher`); `cmd/main.go` (`NATS_CREDS_DIR` env var, `nats.Name("refdata-service")` on every per-tenant connection).
- **Test:** `shared/natstenants/tenants_test.go` — asserts every connection `Manager` opens (PLATFORM and per-tenant) has `nc.Opts.Name` set as given; `NonTenantCredsFiles` excludes `observability`/`sys`/`shipping-admin`/`platform`/`mfe-registry-service` from `Discover`'s result (a `.creds` stem in the shared directory *is* a tenant name unless listed there, so every service credential added to that directory has to be added to the list in the same change — `mfe-registry-service.creds` was not, and every `natstenants`-based service spent its boot opening a bogus tenant connection on the registry's restricted grants). `NonTenantCredsSuffixes` (app-shell Phase 13c) is the same exclusion for a credential *family* that grows: any stem ending `-announcer` is a micro-frontend publisher sidecar, not a tenant — added because five announcer credentials landed in the shared directory in one change and reproduced the same bogus-tenant boot five times over; `EnsureByName` is a no-op (not an error) when the tenant isn't yet visible in `credsDir`; `TeardownByName` stops the adapter and closes the connection, and is idempotent on a tenant never provisioned. `refdata`'s own suite (`refdata/adapter_test.go` et al.) continues to cover BR-D40/BR-D41's refdata-specific behavior atop the shared manager.
...
`rpc.*` is untouched by this split: `internal/natsrpc`'s existing subjects stay exactly as they are (BR-D25), consumed only by other backend services (`shipping-service`'s `refdataconsumer`, `organizations-service`'s `refdataclient`) which were never subject to a browser-token permission concern in the first place.
...
> **Amendment — `frontend/refdata`'s own PLATFORM admin credential.** `frontend/refdata` (the dictionary/corpus editor) turned out to be a cross-tenant, platform-operator tool with no tenant/account concept of its own — it edits `_platform`'s shared standards and every tenant's contexts alike, exactly like the Admin UI, not like Sea Freight Flow. Neither other credential fits that full editor surface: `MintBrowserToken` is denied from `admin.>` by this rule, and a tenant-scoped credential with that deny lifted would conflate refdata-admin rights with tenant membership; `MintAdminToken` permits only Admin's three exact read-only refdata subjects. `accounts-service/auth/token.go` therefore provides **`MintRefdataAdminToken`** — a distinct profile under the same PLATFORM account, scoped to the full `api.*.refdata.>` surface (business and admin — never bare `api.>`), plus `Sub` on `notify._platform.refdata.>` for change notification. `refdata/composition.go` mounts `internal/browserrpc`'s `Adapter` on refdata-service's PLATFORM connection alongside its `rpc.*` adapter. Live-verified: refdata-service shows the PLATFORM `rpc.*` and `api.*` service instances in Admin, and a full corpus create-draft → publish → rollback cycle succeeded end-to-end through `frontend/refdata`.
...
### BR-D45 (Phase 43a, IMPLEMENTED 2026-08-25; reworded Phase 43d 2026-08-25) — This service's `evt.*` and `notify.*` publishes both get the `obs.pubsub.*` hook of `BUSINESS_RULES-SHIPPING.md`'s BR-045

Two publishers in this service, each instrumented inside its own family's seam:
...
- **`notify.*` — in the seam.** `notifybridge.Changed` builds this service's one shape, `notify.{context}.refdata.{typeKey}.changed`, paired with the tokens it is observed under; `composition.go`'s `tenantPublisher` publishes it through a `shared/natsnotify.Notifier`. Phase 43a wired this individually, because the family had no seam then; Phase 43d gave it one. This site was missed in the rule as originally drafted (ADR-047 A2).
...
**Which connection publishes is load-bearing, and this service is the reason the seam holds one.** `notifybridge` holds only the `Publisher` *port* and no connection at all; the fan-out onto each tenant's own `*nats.Conn` happens in the port's single implementation, `refdata/composition.go`'s `tenantPublisher`. The observation must be emitted there, once per tenant publish, because emitting on the tenant's connection is what puts the envelope inside that tenant's account, so BR-AC34's import remap names the right tenant. An emit from the bridge would have had no tenant connection to publish on and would have been unattributable — the same provenance mistake ADR-047 A1 corrected on the import side.

Phase 43d made that structural rather than remembered. A `natsnotify.Notifier` **is** a connection plus an observation gate, so publishing on the wrong one is not expressible; the alternative design, where `Publish` took a connection per call, would have handed the connection back to every caller — precisely what this rule says gets misplaced. This fan-out is consequently the one site in the repo that builds a `Notifier` per message rather than at construction, one per tenant, and the code says why.

- **Enforced in:** `shared/jstream` — `WithObservation` and the observation in `Publisher.PublishWithTrace` (the `evt.*` seam; Phase 43e merged this service's copy of it with shipping's into one module, so "mirroring shipping's exactly" is now a fact about the code rather than about two files kept in step by hand). Opted in at construction in `refdata/composition.go`'s `Startup`, which Phase 43e widened to take the `*nats.Conn` JetStream was built on — replacing the `Handlers.EnableEventObservation` setter `cmd/main.go` used to call afterwards. `refdata/composition.go` — `tenantPublisher.publishTo`'s per-tenant `natsnotify.Notifier`, whose observation is emitted only after that tenant's publish succeeds; `refdata/internal/notifybridge/notifybridge.go`'s `Changed`, which builds the subject and its tokens.
- **Test:** `refdata/evt_observability_test.go` — `TestRefdataChangeIsObservedUnderItsTypeKey` (subject `obs.pubsub.{context}.refdata.{typeKey}.changed`, `Nats-Msg-Id` present for BR-047's dedup, and the causing `rpc.*` span's trace continued). The gate itself — a Publisher nobody opted in emits nothing — is asserted once for both services in `shared/jstream/jstream_test.go`'s `TestPublisherWithoutObservationStaysSilent`. `refdata/notify_observability_test.go` — `TestNotifyFanOutIsObservedPerTenantConnection` (one `obs.pubsub.acme.refdata.port.changed` envelope, carrying the observed subject, emitted on the tenant's own connection — subscribed on `obs.>` so an envelope landing on `obs.trace.*` fails loudly, and asserting exactly one envelope per publish), `TestNotifyObservationIsSkippedWhenTheTenantPublishFails` (a failed tenant publish is not observed) and `TestEvtPublishIsObservedViaTheSeamNotTheCallSite` (a checked convention in BR-049's style: no file in `internal/kvcache` may call `natstrace.Observe*`, so the `evt.*` hook stays in the seam and the next `evt.*` caller inherits it; carrying a span through with `SpanFromContext` is still allowed, and the test fails loudly if the `evt.*` call site ever leaves that package). The fan-out's per-tenant leg was split into `tenantPublisher.publishTo` to make it testable without standing up a tenant `Manager`. The `notify.*` site is additionally held by BR-049's scan, which allows `notifybridge.go` to name the subject and requires that it not publish it.
...
here rather than hardcoded in `organizations-service`. The consuming
