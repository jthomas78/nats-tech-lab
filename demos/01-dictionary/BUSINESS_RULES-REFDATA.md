...
### BR-D29 (Phase 12.10, 2026-07-24) — `obs.rpc.*` is retained on a bounded `RPCTRACE` stream so a reconnecting Admin UI tab can catch up on the last 10 minutes
...
- **Enforced in:** `refdata/composition.go` `Startup()` provisions `RPCTRACE` (`RPCTraceStreamName`/`RPCTraceMaxAge` = 10m) alongside the `REFDATA` change stream, guarded by the same `js != nil` nil-safety as every other JetStream-dependent dependency; `natsrpc.Adapter.publishObs()` publishes via `a.js.PublishAsync` when `a.js` (from `Deps.JS`, threaded through `MountRPC`) is configured, falling back to the original `nc.Publish` otherwise (e.g. tests that construct an `Adapter` without a JetStream connection)
- **Consumer side:** `shipping-service`'s `/api/rpc-watch` SSE handler (`watchRPCObs`) opens an ephemeral ordered consumer on `RPCTRACE` with `DeliverAllPolicy` and drains whatever it currently retains *before* falling through to its existing live `obs.rpc.>` core subscribe — the live subscribe is established first so nothing published during that drain is missed. Best-effort like the rest of this side-channel: a message published in the narrow gap before the live subscribe took effect can appear twice; not worth engineering away for a debug trace feed. Degrades to the original live-only behavior when `h.deps.JS` is nil or `RPCTRACE` doesn't exist yet.
- **Test:** `NATS RPC Adapter / BR-D29` — a fresh ordered consumer created after an `rpc.*` call completes still sees both the request and reply `obs.rpc.*` events (proving retention/replay), while a live core subscriber established beforehand also still receives them (proving BR-D26 is unaffected)
...
### BR-D36 (Phase 17a) — Every `obs.rpc.*`/`obs.api.*` event carries its headers, a publisher-side timestamp, and its payload size
...
- **Enforced in:** `natsrpc.Adapter.publishObs()` (refdata) and `browserrpc.Adapter.publishObs()` (shipping) — both populate the three fields identically, mirroring the existing dual-adapter parity pattern (BR-D26/BR-D27); both adapters' `respondError()` also sets the real error headers on the outgoing `req.Respond()` call.
- **Test:** `NATS RPC Adapter (Phase 12.10) / BR-D36` (`refdata/natsrpc_test.go`) — a request-side event carries the caller's real headers, a non-zero timestamp, and the true payload size; a failed reply carries real `Nats-Service-Error`/`-Code` headers on both the obs event and the actual wire reply; an old-shape envelope with none of the three fields still decodes without error.
