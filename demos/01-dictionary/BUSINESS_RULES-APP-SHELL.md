...
| BR-AS54 | **Domain half done 2026-09-01** in `registry/unregister_test.go`: the action, the plugin, the publisher and the signing key are all inside the signed bytes, so an announcement replayed here is refused as "not an unregister" and a signature cannot be lifted onto a request naming another key; ownership, key state and signature reuse Phase 7's gate unchanged; an older release is refused, the release the running announcement spent is refused as reused, and a duplicate delivery of the accepted unregister is a no-op. **Still to do in 5b:** transport, commit-time trust, audit rows and the real-broker check that no browser credential can send one. Remaining required cases — registry unregister transport/integration specs: owned signature succeeds; wrong action, owner, key, revoked trust, replay, duplicates and stale order fail safely; trust revoked before commit; precise accepted/refused audit and revision; real broker denies browser unregister. Shell: service stop/missing data never withdraw. | 5b |
| BR-AS55 | **Domain half done 2026-09-01** in `registry/unregister_test.go`: an accepted unregister sets a store-owned `withdrawn` beside `enabled` and leaves approval, row, contributions, signed manifest and history alone; it advances the release so a stale return cannot undo it; it is refused outright for an unknown id; a static or unclassified entry is ignored, because curation outranks a publisher; repeat withdrawal is safe. A same-origin return clears the withdrawal, an operator-disabled entry stays disabled and still withdrawn, and a cross-origin return goes back for approval without restoring availability. **Still to do in 5b:** persistence, restart, concurrent operator writes, and an operator enable clearing the withdrawal. Remaining required cases — unregister/reannounce transaction/restart specs: approval retained; operator disable wins either write ordering; stale return cannot undo later unregister; unknown ID cannot gain approval; same-origin trusted return allowed, cross-origin pending, revoked return refused. | 5b |
