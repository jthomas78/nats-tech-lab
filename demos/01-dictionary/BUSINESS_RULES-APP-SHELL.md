...
- **BR-AS62 — A plugin declares its backend dependencies; an operator approves what may be probed.**
  A plugin names, inside its signed manifest, the backend service ids its health decoration should
  reflect. That is a **request, never a grant**: nothing is dialled until an operator approves it on
  the catalogue entry, and only the approved list is probed. Approval is platform-owned curation —
  it is not in the signed bytes, a manifest that asserts it is refused, and approving does not
  un-attest the plugin. An approval is always a subset of what is still declared, so a publisher
  that drops a service from its manifest drops its probe with it, and a re-announce carries the
  approval across rather than revoking it. Declarations are capped (16) and each id must be
  subject-safe, checked where the rest of the entry's structure is checked. **Three answers stay
  distinct:** never declared → `not configured`; declared and not yet approved → `not configured`
  (nothing is dialled on a declaration alone); declared as an empty list, or approved as none →
  `not applicable`. Keep per-service readiness results: any unavailable dependency makes the backend
  summary unavailable; otherwise any unknown/stale dependency makes it unknown/stale; all healthy
  makes it healthy. Presence alone is insufficient; malformed responses, timeouts and no responders
  cannot be reported healthy. No browser `rpc.>` and no new broad registry grants.

  **Amended 2026-09-02.** This replaces the deployment-owned target map
  (`REGISTRY_HEALTH_TARGETS`), which was configuration for a good reason and in the wrong place: it
  lived with the deployment rather than with the plugin, so shipping a plugin meant editing someone
  else's compose file, and forgetting was silent — an unlisted plugin read `not configured` forever,
  indistinguishable from one nobody had got round to. The threat model is unchanged and is why the
  approval exists: a publisher that could name its own probe target could point the registry at a
  service it does not own and read the answer back through the decoration, and the registry's grant
  is one token wide (`rpc._platform.health.*.ready.v1`) across every service. A signature does not
  move that gate — it proves who said a thing, not that they may say it — so the statement and the
  authority are separated instead of collapsed. Static and dynamic plugins now work the same way,
  because both are catalogue entries. The Admin UI spells the middle answer `awaiting approval`,
  which is the operator's word for the same state the shell must call `not configured`: the shell's
  vocabulary is closed and the actionable wording belongs on the screen that can act.
...
| BR-AS62 | **Rewritten 2026-09-02** (see the rule above); originally done 2026-09-01. The deployment map is gone: `REGISTRY_HEALTH_TARGETS`, `domain.HealthTargets` and `registry.ParseHealthTargets` were **deleted**, and a target is now `entry.EffectiveBackendServices()` — the intersection of what the plugin declared in its signed manifest with what an operator approved on the entry. The approval is a lifted, nullable `approved_backend_services` column (NULL = unanswered, `[]` = answered with nothing), because a signed row is rebuilt from its manifest bytes on every read; it is in `CuratedFields()`, cleared by `WithoutCuration()`, refused in a manifest by `ParseManifest`, and carried across a re-announce by `carryApproval` before the branch cascade so convergence still sees the same value on both sides. Specs in `registry/health_backend_test.go`, `curation_test.go`, `manifest_test.go`, `preload_test.go`, `store_integration_test.go`, `lab-shell/src/shell/registry/pluginServices.spec.js` and `admin/src/components/FrontendPluginsPanel.spec.js`. The rest stands as built 2026-09-01: presence is not readiness (`natsready` runs `db.PingContext` on every ask with a 2-second deadline and caches nothing), mixed dependencies aggregate to the worst signal, and `rpc._platform.health.<service>.ready.v1` is in neither browser profile. | 5d, rewritten 15i |
...
## Phase 16 — `plugin-source: build`: one shell, two catalogue sources

**Status: APPROVED at the design gate (2026-09-23), built and verified 16a–16j
(2026-09-24).** Eight rules, BR-AS75 to BR-AS82, copied here **verbatim** from
`Application-Shell-Microfrontend-Plan.md`, which held the wording of record
until this task (16i) ran. No earlier BR-AS rule is amended by this phase: the
split was cut so that `registry` mode keeps every rule it owns, unchanged.

Read them as one answer to one question — **the shell has two ways to find its
plugins, and a plugin cannot tell which one found it.** BR-AS75 picks the
source; BR-AS76 says what the `build` source is made of; BR-AS77 says where its
files are served; BR-AS78 is the promise that makes the pair safe; BR-AS79 and
BR-AS80 keep readiness and health honest across both; BR-AS81 says where the
choice may be spoken aloud; and BR-AS82 says how an embedded plugin reaches its
own demo's backend without any of that being undone by a CORS grant.
...
- **BR-AS77 — Build-mode plugins are served under one public path layout, and the prefix covers the
  ...
  A `registry`-source container **must** fill the same prefix too, and **must** do it without
  packaging any plugin output, because BR-AS03 forbids that. It **must** forward the prefix to an
  upstream the **demo itself** declares, in its own `demo.json`, and that declaration **must** be a
  bare origin — no path — so the demo's own `base` and the shell's prefix cannot drift apart. The
  generated rule **must not** be emitted in `build` mode, where the shell packages the files
  itself: nginx matches the longest prefix, so one emitted rule would shadow the packaged copy. A
  request for an asset that does not exist **must** answer `404`, never the shell's SPA page. The
  upstream **must** be resolved per request, not at start-up, so a stopped demo is a `502` on its
  own prefix and not a shell that refuses to start. Neither side **may** join the other's private
  network; the two **must** meet on a separate network that both opt into explicitly.
...
### How Phase 16's rules are checked

Every row names a check that runs today. The Vitest suite is
`npm --prefix lab-shell test` (809 specs, green 2026-09-24); the two
command-line gates are `npm --prefix lab-shell run lint` (ESLint **and**
`tools/frameOwnership.js`) and `node lab-shell/tools/hostBundleFingerprint.mjs`.
The fingerprint tool has no npm script on purpose — it is a two-step
record/verify ritual, and both steps must run in the **same** `plugin-source`
mode, because Vite inlines the variable at build time.

| Rule | Specced by |
| --- | --- |
| BR-AS75 | `lab-shell/src/shell/pluginSource.spec.js` — the source is read from one explicit variable (`VITE_PLUGIN_SOURCE` yields `build` or `registry`; unset resolves to `registry`; an unknown value **fails boot** rather than falling back; and the resolution is *not* conditioned on a development build, which is what makes `build` a legitimate deployment). Second block: it resolves **once** — the value is memoised so a later environment change is ignored, and the reader takes no plugin argument, so it cannot vary per plugin. Same-origin `remote.url` is held by `pluginAssets.spec.js`'s public-path block and by `buildCatalogueScan.spec.js`, which proves the generator emits relative URLs and stamps no origin. `registry` mode's own origin handling is proven untouched by `preloadFixture.spec.js`'s BR-AS20 block, which still runs unchanged. **Task 16k** added the gate the rule always implied: `buildCatalogueClient.spec.js` — `the same-origin gate on a build catalogue` — a catalogue entry naming another origin is refused before admission, along with a protocol-relative authority, a relative path, a climb out of the plugin's prefix (plain and percent-encoded), the bare directory, and another plugin's prefix. Refused **per entry, never per document** (BR-AS13's tolerance table), with the refusal reported rather than silent, and a malformed remote left to `validateManifest`. Registry mode does not pass through this client at all, so its curated origins are untouched. |
| BR-AS76 | `lab-shell/src/shell/registry/buildCatalogueScan.spec.js` — `manifest.json` is the opt-in; the scan reports every file it read so the dev server can watch them; manifests pass through untouched; an unparseable one is fatal. `the catalogue document` asserts the generated document passes the shell's **own** registry validator, is valid with zero plugins, and is never `degraded`. `buildCatalogueClient.spec.js` holds the distinction the rule demands: a correctly generated **empty** catalogue is a successful empty read, while missing, unparseable, wrong-shape and unreachable each carry their own failure code and render differently through `decideRead`. The no-repo-write clause is asserted in `pluginAssets.spec.js` (`writes nothing on dev-server shutdown`) and in `demoReadinessGeneration.spec.js`. BR-AS03 is proven unchanged in both sources by `node lab-shell/tools/hostBundleFingerprint.mjs --record` / `--verify`, which also asserts the host bundle names no plugin container, remote URL or module path; the baseline after 16k is 17 host assets, digest `7e6a007a2e92e58d321a9f6b59b4ba8190dc937083e5d424e33e6777c13c3bf0`. |
| BR-AS77 | `lab-shell/src/shell/registry/pluginAssets.spec.js` — the public path layout (`/plugins/<id>/`, relative to whatever origin serves the shell); the development proxy, derived from the **same scan** as the catalogue, upgrading WebSockets so HMR works through the prefix and rewriting no paths; the dev middleware for a demo with no dev server, which serves the **whole** built plugin at its public path, 404s a missing asset instead of returning the SPA page, and refuses path traversal; and the build-time collection, which copies each opted-in demo into the served tree and **fails the build** when a catalogued demo has no `dist/` — emitting the catalogue alone is not sufficient. The separation of readiness routes from the asset prefix is in `demoReadinessGeneration.spec.js` (`never under the plugin asset prefix`, plus `the prefix closed with 404 so nothing falls through to the SPA`). Verified live on demo 04 in 16c, entry and lazy chunks both. `pluginAssetPath.spec` coverage is joined by `buildCatalogueClient.spec.js` from **task 16k**: `isPluginAssetUrl(id, url)` is the prefix asked as a question, and the build-mode client refuses any `remote.url` that is not under the plugin's own `/plugins/<id>/` prefix. **That gap is CLOSED by task 16l** (it was carried out of 16k as finding 2). `demoAssetsGeneration.spec.js` covers the generator: the demo-owned `assets.hostedUpstream` declaration, eight malformed shapes refused, one `location` per declaring demo, the deferred resolver lookup, no status interception, and **nothing emitted in `build` mode** — which is how build-mode packaging is preserved, since nginx's longest-prefix match would otherwise let the proxy shadow the packaged copy. `hostedPluginAssets.spec.js` covers the arrangement against the **real repo**: each declaring demo ships its own frontend image whose nginx ends `=404` with no SPA fallback, its files sit under `/usr/share/nginx/html/plugins/<id>` with a matching Vite `base`, the shell's `nginx.conf` includes the generated rule and names no demo, the shell's `COPY` lines reference no `frontend/dist`, and the edge network is `external: true` on both sides, absent from both non-overlay composes, and carries only the demo's frontend and command API. Verified on the packaged image against the live cell: entry, CSS, a lazy chunk and a font all `200`, a missing asset `404`, and the shell cold-starting to `200` with no demo container present. |
| BR-AS78 | `lab-shell/src/shell/registry/buildCatalogueScan.spec.js` — `leaves manifest.json untouched, so BR-AS78 still holds`: the sibling `demo.json`'s dev port must not leak into the manifest, asserted by deep equality against the original and by the serialised manifest not containing the port. `demoReadinessGeneration.spec.js` — `leaves manifest.json byte-unchanged`: the scanned plugin re-serialises to the identical bytes, and the result still validates as a registry document, which is what keeps `registry` mode's drift check green. `demos/04-jetstream-cqrs/frontend/src/plugin.spec.js` — `the plugin's identity, written in three files`: the same federation container in the manifest and the build config, every asset built under the manifest's public path, and the dev-server port kept out of the manifest. `preloadFixture.spec.js` carries demo 04's curated registry row built from that same unmodified manifest. **Known limit, recorded rather than claimed:** no spec diffs the manifest file's bytes across a `build` run and a `registry` run; the guarantee is asserted structurally on the scan output and by the registry's existing drift check. |
| BR-AS79 | `lab-shell/src/shell/demos/demoGate.spec.js` — the gate mounts a ready demo, draws the **shell's** own panel when unready rather than letting the plugin mount and fail, never says "stopped" for an unreachable demo, checks once on open, retries in place and mounts on a changed answer, steps aside once mounted, and is not applied at all when there is no demo store. `demoReadiness.spec.js` — the three states end to end: `available` only on an explicit demo assertion, while a bare 200, no verdict, a refused connection and a timeout all read `unknown`, and a 404 is a missing route rather than a stopped demo; the menu card's own refresh policy (`populates every card without opening a demo`, one in-flight request shared, no duplicate timer); the declared audience (F-5), which defaults to `visitor`, fails boot on an unknown value and is **never inferred from build mode**; and `the readiness layer and the plugin source`, which proves the same route is built whichever source found the plugin and that no plugin source is named anywhere in the layer. `demoReadinessGeneration.spec.js` — the declaration lives in a sibling demo-owned file, is optional, is normalised once, cannot admit a plugin or supply a `remote.url`, and leaves `manifest.json` byte-unchanged; plus the dev proxy and the hosted nginx snippet, one exact-match route per demo, not widenable by suffix. The same-origin, no-CORS clause is held from the demo's side by `demos/04-jetstream-cqrs/cqrs/ready_api_test.go` — `sets no CORS header for any origin`. One shared presentation component is enforced by `lab-shell/tools/frameOwnership.js`, whose single named exception lets `demos/demoGate.js` — and only it — import `@ui-shell/DemoStatePanel.vue`; `frameOwnership.spec.js` asserts both halves of that exception. **Task 16k** closed a hole in the timeout: `demoReadiness.spec.js` — `a body that never finishes arriving` — `fetch` resolves on the response HEADERS, so a demo that stalled mid-body left the read pending past `timeoutMs` with no timer left to abort it, and every later check queued behind it. The timeout now spans the body read, reports `unknown` / `timeout`, and always clears, so the next check recovers. `demoRunCommand.spec.js` holds the other half: a `runCommand` naming a path in this repo must exist and be executable. |
| BR-AS80 | `lab-shell/src/views/PluginsView.spec.js` — `build` mode reads `not configured` on both sides for every plugin, never rests at `unknown`, invents no `healthy` or other measured state, **does not age**, and uses the shared quiet tone; `registry` mode is asserted unchanged in the same file, still saying what was measured, still ageing, still resting unmeasured plugins at `unknown`, and treating a flagless health object as monitored. `healthSourceIndependence.spec.js` is the structural half: `healthPlane.js` and `healthText.js` contain no `pluginSource`, `PLUGIN_SOURCE` or `VITE_PLUGIN_SOURCE` and never read the `monitored` flag, so the measurement layer cannot become source-aware; and `main.js` gates the plane, the epoch watch and both timers on `monitored` while putting **nothing** into the signal map, so no `HealthSignal` is synthesised. That readiness and health stay separate is held by `demoReadiness.spec.js`'s vocabulary block and by the `frameOwnership.js` exception being scoped to one file and one specifier. Verified live in both modes at 1920×1080 on 2026-09-24. |
| BR-AS81 | `lab-shell/src/views/PluginsView.spec.js` — `the catalogue source statement`: the source is named **once** at page level (one `.source` node, inside `.page-head`), never repeated in a row, worded as a property of this shell (`this shell is configured to read its catalogue from …`), and not shown alongside the registry's `announced` / `preload` labels. `labDemos.spec.js` holds the other half of "not on the cards": the demo menu's entries carry no health, no readiness and no `plugin-source` field. `demosReadme.spec.js` — the generated `demos/README.md` states source as a property of the shell rather than of a demo, gives a frontend-less demo nothing to source, and its last spec regenerates from the working tree and **fails when the committed page is stale**, which is what stops a hand-restated source creeping back in. The generator itself is `npm --prefix lab-shell run demos:readme` (`--check` to test without writing). |
| BR-AS82 | `lab-shell/src/shell/demos/demoApiGeneration.spec.js` — the `api` declaration is optional, normalised once, cannot admit a plugin or supply a `remote.url`, needs an upstream as well as routes, and drops four unsafe route shapes (a relative path, `//host`, a `..` climb, a query). `the development proxy` holds the property the rule rests on: one entry per **declared route**, and `/readyz` — which this demo's backend really does serve, and really does declare under `readiness` — is refused here, as are an undeclared route, a suffix on an exact route, the bare base and a climb; the origin presented upstream is unchanged, so no CORS grant is needed by anyone; and the prefix is never under the asset or readiness prefixes. `the hosted nginx snippet` — one `location` per named route, exact unless the demo asked for a prefix, each upstream behind a variable and a resolver so a stopped demo cannot stop the shell, and the prefix closed with `return 404` so nothing falls through to the SPA. From the demo's side, `demos/04-jetstream-cqrs/frontend/src/plugin.spec.js` — `the command API base, when the demo is embedded`: absolute until the shell activates the plugin, then moved to the shell-origin path, with the live-binding requirement stated where breaking it would show; and the NATS WebSocket URL deliberately **not** moved. The no-CORS clause is held from the backend's side by `demos/04-jetstream-cqrs/cqrs/serve_test.go`, whose `testOrigin` is still `http://localhost:20401` and nothing else. |

**Where the two inventories differ, and why that is correct.** The demo menu
lists the lab's demos; the Plugins screen lists the active catalogue's plugins.
Demos 02 and 03 have no frontend, so they have no plugin, so they appear on the
menu and in no catalogue — as shell-owned intro pages at `/lab-demos/:demo`,
carrying their run commands and the paths to their findings, and carrying **no
...
- **BR-AS91 — A route record hands the plugin its own local contribution id.** Every plugin route
  record the shell builds **must** pass the route's own **local** contribution id to the component
  it renders, as a `routeId` prop, alongside the route params. One plugin component **may** sit
  behind several of that plugin's routes, and it **must not** have to ask a router which one it is
  showing: `vue-router` is the shell's own dependency and is **not** a shared federated module, so
  a remote that imported one would load a second router with no history of its own (BR-AS07).
  - **Every plugin route gets it, and the shell names no plugin doing so.** The prop **must** be
    added by the one funnel every route contribution passes through, whether the component reads it
    or not. No demo id and no per-demo branch **may** appear (decision D17-7).
  - **The LOCAL id, never the qualified one.** The value **must** be the word the manifest author
    wrote (`lesson-02`), not the shell's globally unique `pluginId/localId` (BR-AS06). The plugin
    reads its own vocabulary back.
  - **Params are not lost, and a param of the same name does not win.** The record **must** still
    pass the route params it passed before, and `routeId` **must** be applied after them, so a
    path segment cannot shadow the shell's own answer.
  - **It survives the readiness gate.** The value **must** reach the plugin component through
    `withDemoGate`, which renders it by hand, so a gated demo and an ungated one behave the same
    (BR-AS79).
  - **A cold link is the point.** A component that chose its own default instead **would** render
    the wrong screen for a pasted link, a refresh or a browser back, with the address bar lying
    about it (amendment A4).
...
| BR-AS91 | *(17h)* `lab-shell/src/shell/routing/shellRoutes.spec.js` — the props function: the local id handed over and not the qualified one, every plugin route getting it, params still passing through, and a same-named param not winning. And against a real router with the gate installed: a cold link to lesson 02 opening lesson 02, a cold link to lesson 01 opening lesson 01, browser back and forward following between them, the value surviving `withDemoGate`, and readiness probed once per PLUGIN across both lessons. Plus `demos/04-jetstream-cqrs/frontend/src/plugin.spec.js` — the component showing the lesson the route names, naming it in its breadcrumb, falling back to the first lesson when the route says nothing, importing no router, and drawing no rail of its own. |
...
### Phase 17's acceptance checks, and where each one is proven

The ten checks were listed at gate time so they could not be negotiated down later. Task 17i
closed the four that no earlier task had reached, and this table is the record of all ten. Where a
check reads "*(17i)*" the spec was written for this table; the rest were already standing and are
named here so the list is complete rather than re-argued.

| Acceptance check | Proven by |
| --- | --- |
| Build/registry parity — the same manifests, the same tree, the same clashes, from either source (D17-3) | *(17i)* `lab-shell/src/shell/contributions/catalogueParity.spec.js` — raw manifests fed through the REAL clients, `createRegistryTransport` on one side and `createBuildCatalogueClient` on the other, in deliberately OPPOSITE orders, then validated and indexed. The grouped tree, the clash list with its participants, and the refusals all compare equal. A fourth spec reads the tree out in full, so the parity cannot pass by matching nothing against nothing. |
| The two-plugin example at the head of the phase renders exactly as drawn | *(17i)* `lab-shell/src/shell/ui/shellNavSections.spec.js` — the drawing itself: one `JETSTREAM` band holding Lesson 1, Lesson 2 and Lesson 3 in that order, then `TOPOLOGY` holding 3 NATS Cluster; the band shown once under the agreed label; each item still owned by the plugin that gave it; and nothing marked, because the two plugins agree. |
| Two plugins, one `group.id`, different `group.label` — both rendered, both marked | *(17c/17d)* `navigationTree.spec.js` (both entries placed, one displayed name chosen, the losing label kept with its claimant) and `navigationClashes.spec.js` (`group-label-conflict`, naming both owners). |
| Two groups, different `group.id`, same visible label — both rendered, both marked | *(17d)* `navigationClashes.spec.js` — `duplicate-group-name`, with the two band ids named. |
| The same child label in one band at different routes — rendered twice, marked | *(17d)* `navigationClashes.spec.js` — `duplicate-nav-label`, with the silence beside it: the same name at the same route says nothing. |
| Withdrawing a plugin updates the merged band live, with no empty band and no dead link (BR-AS56) | *(17f + 17i)* `shellNavSections.spec.js` — the empty band dropped and restored, and *(17i)* the MERGED case: a band shared by two plugins keeps standing when one withdraws, goes only when the LAST owner goes, leaves every surviving item with a real destination, comes back on restore, and re-picks its label from the owners that remain. |
| A nav entry whose route was refused leaves a visible diagnostic and NO clickable dead link | *(17i)* `shellNavSections.spec.js` — the rail half: no item drawn for it, every item that IS drawn holding a destination, an `unresolved-route` refusal on the record naming the entry, the rest of the plugin still placed, and no clash reported. `lab-shell/src/views/PluginsView.spec.js` — the screen half: the entry named on the Plugins screen by qualified id and by cause, in the refusals list and not in `ul.clashes`. |
