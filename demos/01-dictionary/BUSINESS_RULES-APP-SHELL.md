...
| --- | --- | --- |
| BR-AS52 | **Service half done (2026-09-01):** `registry/lifecycle_test.go` — the two classes are the only ones a write may state, an unclassified row is read as static, and a migration backfills a legacy row without touching enablement, withholding, the signed bytes or the revision; `phase8_integration_test.go` amended so the unclassified case asserts the backfill. **Still open:** shell `manifestSchema.spec.js` (source does not imply class), Admin `FrontendPluginsPanel.spec.js` (display/edit class), `registryDiff.spec.js` (held class survives edits until reload, including edit then disable). | 5a |
