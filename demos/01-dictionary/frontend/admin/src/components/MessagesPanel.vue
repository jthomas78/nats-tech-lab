...
const { upsertMessage: feedUpsert, connected, bootstrapFailed } = usePubsubFeed({
...
<template>
  <div class="lab-panel msg-card">
    ...
    <p
      ...
      class="err-line"
    >
      Message snapshot failed to load — this view may be incomplete. Retrying…
    </p>
    <p
      v-else-if="!connected"
      class="stale-line"
...
.msg-table :deep(.p-datatable-tbody > tr.p-datatable-row-selected > td) {
  background: rgba(0, 111, 255, 0.08);
}
.msg-table :deep(.p-datatable-tbody > tr.p-datatable-row-selected > td:first-child) {
