...
> **BR-AC35 extension (2026-08-25):** this rule's identity pair now also
> covers this service's **REST** entry point. `httpTraceMiddleware` records a
> responder derived from the tenant connection's own `nats.Name`
> (`natstrace.ResponderHeaders`, since a REST hop has no `micro.Service` to
> read an identity off) and lifts the browser-declared `Nats-Requestor` off
> the inbound HTTP headers. Format and BR-041's observability-only constraint
> are unchanged; the one asymmetry is that a REST responder identity exists on
> the span only, never as a response header — see
> `BUSINESS_RULES-ACCOUNTS.md`'s BR-AC35 for the full rule.

- **Enforced in:** `dictionary/internal/browserrpc/adapter.go`'s `respond()`/`respondError()` (set `Nats-Responder`) and `micro.AddService`'s `Config.Name`; `internal/refdataconsumer/consumer.go`'s `New()`/`requestRPC` (sets `Nats-Requestor`); `frontend/seafreight-app/src/nats/useNatsConnection.js`'s `request()` (sets `Nats-Requestor`); `dictionary/internal/rest/trace_middleware.go` (REST, BR-AC35)
...
> **2026-08-26 amendment — in the Connections *detail pane*, the raw-NKey fallback is gone; the NKey is always its own row.** The rule above says both panels prefer the resolved label "falling back to a truncated raw NKey otherwise." That remains true of the table column, which has one cell to spend and must show *something*. It is no longer true of the detail pane: `Account` there renders the label alone, or an em-dash when none resolved, and the raw key moved to a dedicated `Account NKey` row beside it. Two reasons, and the second is the one that decided it. First, the pane has the room the column doesn't, so collapsing two different facts into one line buys nothing there. Second, the pane now also carries BR-058's `Credential` / `User NKey` pair, which establishes exactly this shape — friendly value on one row, the raw NKey behind it on the next — and leaving Account as the single row that stacks both would have made the pane inconsistent with itself. The em-dash is the honest reading of "wherever possible": an unresolved account has no friendly name, and showing its key in the row labelled `Account` implied the key *was* the name. Nothing about the resolution mechanism changes — this is a rendering change in `ConnectionsPanel.vue` only.
...
- **Enforced in (Phase 28i):** every service's `natstrace.go` — `StartFromHeaders`'s `reqHeaders` retention, `Span.SetRequestHeaders`, and `redactHeaders` in `finish()`; the three outbound call sites (`internal/refdataconsumer/consumer.go`, `organizations/internal/refdataclient/client.go`, `accounts/handler.go`'s `publishAccountEvent`).
...
> **`consumer.durable` amendment (2026-08-27) — a projector span now names the durable consumer that produced it, because "one span per message" is not "one span per event."** The Phase 28d amendment above is precise about the invariant it establishes (each `Consume` callback publishes exactly one span per message it processes) and `trace_async_test.go` pins it per projector. What neither says, and what the wire span could not express, is that **two** of the three projectors filter the *same* subject: `container-projector` (`container_handler.go`) and `meta-projector` (`meta_handler.go`) both consume `evt.*.shipping.container.>`. One `RegisterContainer` command therefore publishes one `.registered` event and produces **two** spans — same `traceId`, same `subject`, same `entity_id`, differing only in `spanId` — which rendered in `TraceWaterfall.vue` as two identical sibling rows and read as a duplicate publish rather than as two consumers of one message. (`.loaded`/`.unloaded` do not double up: `meta_handler.go` acks and returns *before* starting a span for any event type other than `registered`.) Each of the three callbacks now records a `consumer.durable` attribute — `ship-projector`, `container-projector`, `meta-projector` — carried in BR-036's existing `attributes` map, so no wire field was added.
>
> **Deliberately an attribute and not a header, in either direction.** `Nats-Requestor` was structurally impossible: the event is published once, before any consumer exists, so the publisher cannot name its readers — and BR-041 filters whole traces by that header, which a consumer name would corrupt with values that are not callers. `Nats-Responder` was the right *role* (the projector is the party that handled the message) but the wrong *field*, twice over: `ResponderIdentity` is `"<name>/<instance ID>"` where the instance half is minted once per **process** (BR-027), and both container-subject projectors run in one process on one tenant connection — so both would stamp the identical value and stay indistinguishable, unless the format grew a third segment and broke what BR-027 and the Admin UI's Connections panel read. Worse, `TraceWaterfall.vue`'s `respondedByEmptyLabel` deliberately renders an `evt.*` span's missing responder as `"async event — no NATS responder"` — a JetStream consumer reacting to a message never had a responder to record — with a spec pinning that exact string; stamping one would have fabricated a reply that is never sent, the same class of mistake as BR-036's Phase 30j amendment (echoing event bytes back as a reply `payload`). The attribute keeps both facts true at once: the span names its consumer, and the identity line still says no responder existed.
>
> The durable names are declared once as `eventhandler.ShipProjectorDurable`/`ContainerProjectorDurable`/`MetaProjectorDurable` and used both as the `ConsumerConfig.Durable` and as the attribute value, so a trace row, `nats consumer ls`, and the Admin UI's Consumers panel cannot drift apart — a stale copy would mislabel a projector rather than fail loudly. The key is dotted (`consumer.durable`, not a bare `consumer`) to match `http.method`/`http.path`/`rpc.retry_count`, and **no UI change was required**: `TraceWaterfall.vue`'s `splitRows` routes every attribute absent from `REQUEST_ATTRIBUTE_KEYS` to the response cell of the detail pane, which is where "who processed this" belongs. The waterfall *row* itself still shows no durable — telling the two apart is two clicks, not zero — deliberately left as a separate presentation decision rather than folded into this change.

- **Enforced in:** `internal/refdataconsumer/consumer.go`'s `requestRPC` (mints the child span before its retry loop, sets `traceparent` alongside `Nats-Requestor`, publishes to the tenant-local alias unchanged); organizations-service's `internal/refdataclient/client.go`'s `requestRPC` (same pattern, second of the two Phase 28c call sites); `shared/jstream`'s `Publisher.PublishWithTrace` (Phase 28d; Phase 43e merged shipping-service's and refdata-service's two copies of it into one module and unexported the publish primitives beneath it, so the header is no longer something a caller could route around); organizations-service's `orchestration.JetStreamEventStore.append` (Phase 43e — that seam had gone without a `traceparent` since Phase 28: this rule already required one, nothing enforced it there, and no consumer read the header loudly enough for the gap to surface. See BUSINESS_RULES-ORGANIZATIONS.md's BR-TP75); `dictionary/internal/browserrpc/adapter.go`'s 9 handlers seeding `natstrace.ContextWithSpan` (Phase 28d, Piece 1); `dictionary/internal/application/commands/commands.go`'s `Publisher` interface (widened with `PublishWithTrace`) and `ShipHandler.publish`/`dictionary/internal/application/commands/container.go`'s `ContainerHandler.publish` (Phase 28d, Piece 2); `dictionary/internal/eventhandler/handler.go`'s shared `register()`, `container_handler.go`'s `RegisterContainers`, and `meta_handler.go`'s `RegisterMeta` (per-message spans, Phase 28d Piece 3; the `consumer.durable` attribute and the three `*ProjectorDurable` consts, 2026-08-27), plus their `publishNotify`/`publishRawNotify`; `internal/kvstore/kv.go`'s `Store.publishNotify` (carries trace context on the derived notify, never in the KV `Put` body).
- **Test:** a 3-attempt-then-fail `requestRPC` call asserts exactly one span published with `attributes["rpc.retry_count"] == 2`, not three; `internal/refdataconsumer/consumer_test.go`'s `TestLookupPublishesOneSpanCarryingATraceparentHeader`/`TestLookupContinuesParentSpanAttachedViaContext`/`TestLookupPublishesOneErrorSpanWithRetryCountWhenNoResponder`, and organizations-service's `internal/refdataclient/client_test.go`, pin both the local-alias subject (unchanged) and the span's explicit labeling. Phase 28d: `dictionary/trace_async_test.go`'s "BR-037 (Phase 28d)" specs assert an api.\* command's resulting evt.\* JetStream publish carries the same `traceId` as its reply-side `obs.trace.*` span (including with no inbound `traceparent` at all — a root span), and that each of the three projector `Consume` callbacks (`RegisterShapeA`, `RegisterShapeB`, `RegisterContainers`, `RegisterMeta`) publishes exactly one span per message labeled with the correct `service`/`entity`/`action`/`entity_id`; `shared/natsnotify/natsnotify_test.go` pins the `notify.*` seam's nil-safe header attachment directly (Phase 43d moved these off the per-call-site helpers they were written against); `organizations-service/.../orchestration/pubsub_observability_test.go`'s `TestAppendCarriesTheCausingTraceparent`/`TestAppendOmitsTraceparentWhenNoSpanIsReachable` pin the third seam (Phase 43e); `internal/kvstore/kv_test.go`'s two new `EnableNotify` specs assert `Store.Put`'s derived notify attaches the `Traceparent` header when `ctx` carries a span and omits it cleanly when it doesn't. **`consumer.durable` (2026-08-27):** `dictionary/trace_async_test.go`'s three per-projector specs each assert their own durable on the span alongside `entity_id`, and a fourth spec — "labels the two spans one `.registered` event produces with the distinct durable of each projector that consumed it" — registers **both** container-subject projectors, drives one register through the `api.*` adapter (the only path that seeds a parent span, so both projectors continue one trace rather than each minting a root), and asserts two spans arrive keyed by distinct durables with matching `traceId`/`entity_id` and differing `spanId`. It deliberately does not use the `firstMatching`/`noMoreMatching` helpers: "exactly one" is the wrong assertion there — two is correct, and being able to tell them apart is the point. `TraceWaterfall.spec.js` pins the presentation half: the attribute renders in the response column and *not* the request column, while the response identity line still reads `"async event — no NATS responder"`.
...
- **pricing-service** (`pricing/internal/rest/handlers.go`): `GET /healthz`
  only (BR-P26).
- **organizations-service** (`organizations/internal/rest/handlers.go`):
  `GET /healthz` only (BR-TP16).
- **observability-service** (`observability/internal/rest/handlers.go`): `GET
  /healthz`, `GET /api/nats/connections`, `GET
  /api/nats/connections/closed` (Phase 51a, BR-062), `GET
...
- **The service that owns the real connection fires its own probe.**
  `shipping-service` and `organizations-service` already hold the only
  ...
  permits only three exact PLATFORM refdata reads and cannot publish the
  tenant-local import alias; a PLATFORM-only connection cannot resolve that
  alias by name at any permission level. That is why this cannot be
  `observability-service`'s own publish as originally designed.
...
### BR-045 (Phase 43a, IMPLEMENTED 2026-08-25; reworded Phase 43d 2026-08-25) — `obs.pubsub.{context}.{service}.{entity}.{action}` is published once per `evt.*`/`notify.*` publish, redacted-then-truncated the same way as `obs.trace.*`, and continues the causing span's trace
...
**Each family is instrumented inside its own seam** (ADR-047's amendment A3, extended by Phase 43d):

- **`evt.*`** — `Publisher.PublishWithTrace` (`shared/jstream`, one module for shipping and refdata since Phase 43e) and `JetStreamEventStore.append` (`organizations-service/.../orchestration/event_store.go`, BR-TP75) for organizations. `PublishWithTrace` has exactly three call sites in the whole backend and all three are `evt.*` publishes — it is not a generic primitive, it is already the `evt.*` choke point, one per service.
- **`notify.*`** — `shared/natsnotify`'s `Notifier.Publish`. Phase 43a instrumented this family at each call site instead, because it had no seam; Phase 43d gave it one, and the enumerated call-site list that used to live here went with it.

Coverage is therefore **structural in both halves**: a future publisher in either family is observed by construction rather than by remembering to wire a hook. That is what the rule asserts — a property of every published message, not of a list of places.

**Still never inside a generic publish primitive** (`shared/jstream`'s unexported `publish`/`publishMsg`) — wrapping *there* would risk picking up unrelated JetStream control traffic that happens to share the same underlying client call. That ban is unchanged; it simply no longer extends to `PublishWithTrace`, which the self-observation argument never applied to (`obs.pubsub.*` is emitted over core `nc.Publish`, not through `jstream.Publisher`, so the seam cannot observe its own emission).

**Observation is opt-in, and exclusion is expressed by the gate.** Both seams stay silent until observation is asked for at construction — `WithObservation(nc)`, the same option name on both sides since Phase 43e retired the `evt.*` side's post-construction `EnableObservation` setter. A publisher that is never opted in cannot later become observed by a caller reaching for a setter, which is what makes "excluded" a fact about how the publisher was built. `observability-service`'s two publishers are built without it: `tracestore`'s is that service's internal KV-change plumbing for its own `trace-request-reply` bucket rather than a domain event, and `pubsubstore`'s announces a write to the very bucket these envelopes are stored in, so observing it would be an unbounded feedback loop. Neither needs an entry in a hand-maintained exclusion list any more; "excluded" is now a fact about how the publisher was constructed.

Each envelope derives its `traceId`/`parentSpanId` from the span the call site already had — for the `evt.*` seam that is the `*natstrace.Span` argument `PublishWithTrace` already reads to attach the `Traceparent` header — rather than minting a new, unrelated trace, so a future UI could correlate "this event was published as part of trace X" without any new plumbing. With no span reachable (a nil argument) the observation roots its own trace rather than being dropped.
...
**The observation is emitted only after the domain publish succeeds.** An event that never reached the stream did not happen and must not appear on the operator's wire tap; a publish that fails returns its error unchanged, with nothing observed.

**The observability tokens are never derived from the subject on the `notify.*` side.** Each service builds its own subjects and hands `natsnotify` a `Subject` carrying both the name and its four tokens. Deriving them positionally is wrong for two members of the family, not merely fragile: `notify._platform.refdata.{context}.…` must be attributed to `{context}` rather than the `_platform` in token 1, and `notify.accounts.account.{action}` is four tokens, below `ObservePublish`'s five-token floor, so it would be skipped outright. The `evt.*` seam continues to derive positionally, where the arities are regular and the tokens do sit where it reads them.

- **Enforced in:** `shared/natstrace/natstrace.go`'s `Tracer.ObservePublish` / `ObservePublishAs` (the emit primitive and its subject derivation — action is the *last* subject token, not a fixed index, so 6-token `evt.*` and 5-token `notify.*` both derive correctly, and `obs.*` subjects are never self-observed). **`evt.*` seams:** `shared/jstream`'s `Publisher.PublishWithTrace` with `WithObservation`, opted in per tenant at `dictionary/internal/rest/tenant.go` and, for refdata, at `refdata/composition.go`'s `Startup` (BR-D45); `organizations-service/.../orchestration/event_store.go`'s `JetStreamEventStore.append` with its own `WithObservation`, opted in per tenant runtime at `transporterprofile/runtime.go` (BR-TP75). **`notify.*` seam:** `shared/natsnotify`'s `Notifier.Publish`, with subjects and tokens built by each service's own subject-builder layer — `shipping-service/internal/notify` (five shapes), `refdata-service/.../notifybridge`'s `Changed` (BR-D45), `accounts-service/accounts/handler.go`'s `lifecycleSubject` (BR-AC34), and `observability-service`'s two `kvChanged` builders, whose Notifiers are constructed without `WithObservation`. Grant: `nats/bootstrap-operator.sh`'s restricted `shipping-admin` user gains `obs.pubsub.>` on allow-pub, without which the bridge's emit is dropped server-side as a Publish Violation.
- **Test:** `shared/natstrace/natstrace_test.go` — "obs.pubsub.* publish observation (Phase 43a, BR-045)" (subject derivation across the three arities, `direction: "publish"`, trace continuation vs. root minting, `Nats-Msg-Id` = `spanId`, redact-then-truncate, the self-observation guard, and `ObservePublishAs` for the context-less `notify.accounts.*` shape); `shared/jstream/jstream_test.go` — the `evt.*` seam's own surface, asserted once for both services since Phase 43e: `TestPublishWithTraceObservesUnderTheDerivedTokens`, `...ContinuesTheCausingTrace`, `...RedactsBeforeObserving`, `TestPublisherWithoutObservationStaysSilent`, and `TestFailedPublishIsNotObserved`; `shipping-service/dictionary/evt_observability_test.go` and `refdata-service/refdata/evt_observability_test.go` — each service's own subject shapes, which is all that stayed behind; `shared/natsnotify/natsnotify_test.go` — the `notify.*` seam's own surface: that it publishes the domain message, observes under the tokens it was given rather than the ones in the subject, observes a four-token subject the positional deriver would skip, stays silent without `WithObservation`, continues the span on `ctx`, attaches and omits the traceparent correctly, and does not observe a publish that never reached the wire; `shipping-service/internal/notify/notify_test.go` — the five subject shapes and their tokens, asserted without a NATS server; `dictionary/internal/eventhandler/platform_notify_test.go` — the refdata bridge's observation under the change's own context; `internal/kvstore/kv_test.go` — "observes the notify publish on obs.pubsub.{context}.kv.{bucket}.changed". `browserrpc`'s site has no package-level test (that package has none at all); it needs none — it publishes through the seam, which is tested once. Sibling services' tests: BR-D45, BR-AC34, BR-TP75.

### BR-046 (Phase 43a, IMPLEMENTED 2026-08-25) — `obs.pubsub.*` payload redaction reuses the existing shared denylist, after a named payload review
...
### BR-047 (Phase 43b, IMPLEMENTED 2026-08-25) — Published `obs.pubsub.*` envelopes are ingested best-effort into their own bounded stream, deduplicated by `Nats-Msg-Id`
...
**Two subject sets, not one — found at implementation time and worth stating.** The stream captures **both** `obs.pubsub.>` and `monitor.*.pubsub.>`. PLATFORM's own services publish on the former directly; every *tenant's* export arrives remapped onto the latter by BR-AC34's `LocalSubject`. Capturing only `obs.pubsub.>` — the obvious reading, and what `tracestore` did for `obs.trace.>` before BR-AC36 gave that import a remap too — would make the stream blind to exactly the cross-tenant traffic Phase 43 exists for.

**Resolved: a KV bucket, not stream-only.** The open question above was whether ingestion needs a bucket the way `trace-request-reply` does. It does, but **not for merging** — a standalone envelope has nothing to merge, so a write is a plain `Put` rather than `tracestore`'s read-modify-write append. It needs one because the bucket *is* the Admin UI's read path: every observability panel bootstrap-fetches a KV bucket and live-subscribes to `notify._platform.kv.{bucket}.>` (`useTraceFeed.js`), and a browser credential is never granted `obs.pubsub.>` (BR-AC34). The bucket is a **bounded visible window** over the stream, so its caps are deliberately tighter than the stream's (15 min / 8 MiB vs. 1 h / 32 MiB) — 43c fetches every entry on load, making bucket size a page-load cost, not just disk.

**The tenant is derived from the arrival subject, not the payload.** `monitor.{tenant}.pubsub.>` yields the tenant; a bare `obs.pubsub.>` arrival was published inside PLATFORM and reports `_platform`. That token is inserted by the NATS server, so a tenant cannot spoof its own provenance by writing a field into the envelope. This is what BR-048's panel reads.

**Caps measured, per this rule — and the ADR's own estimate was 4× high.** A seed run over representative shipping payloads measured a real envelope at **454 B** (`evt` ship.arrived), **550 B** (`evt` container.loaded) and **592 B** (a `notify.*` carrying a whole projected KV value), against the ~2 KiB ADR-047 assumed. At ~600 B, 32 MiB is on the order of 55k envelopes, so the advertised hour of `MaxAge` is a bound this stream can actually honour at demo volume rather than one `MaxBytes` silently overrides first.

**Re-measured 2026-08-25 after 43a's widening**, as this rule required — the first figures came from a tree where only shipping's `evt.*` seam emitted. Across the full instrumented set, with representative payloads:

| Publisher | Payload | Envelope |
|---|---|---|
| refdata `evt`/`notify` — `{typeKey, context, version}` | 55 B | **358 B** |
| accounts `notify` — `{name}` | 15 B | **317 B** |
| shipping `evt` — raw ship.arrived | 86 B | **389 B** |
| organizations `evt` — profile-updated + 3 `FieldChange`s | 326 B | **630 B** |
| shipping `notify` — projected `ShipState`, 24 containers | 541 B | **845 B** |
| shipping KV `notify` — `meta` value, 120 known containers | 1 714 B | **2 019 B** |
| worst case — any payload over the 4 KiB redact-then-truncate cap | 32 KiB | **4 424 B** |

Envelope overhead is a flat **~303 B**; everything else is the payload. Two things follow. **The original 454–592 B range was a shipping-`evt`-only artifact** — the widened set spans 317 B to ~2 KiB, and the biggest publisher is the KV-change notify, which carries a whole bucket value. **The 4 KiB truncation cap is a real ceiling**: no envelope can exceed ~4.4 KiB however large the payload, so `MaxBytes` cannot be blown by one pathological message. At a ~800 B blended average, 32 MiB is ~40k envelopes and the 15 min / 8 MiB bucket is ~10k entries.

**One sizing consequence is open, and is a 43c cost rather than a 43b one.** The Messages panel bootstrap-fetches *every* bucket entry on load but renders at most 500 rows, so a full 8 MiB bucket is a multi-megabyte page load spent to display a few percent of it. The bound is honest and the panel says so, but the fetch is not yet bounded to what the panel can show — tightening `BucketMaxBytes`, or giving the bootstrap a limit, is a change to both BR-047's caps and 43c's feed and is deliberately not made here.

**Malformed envelopes are acked, not Nak'd.** Nothing about a redelivery makes unparseable JSON parse, so a Nak would redeliver it forever behind traffic that is fine.

- **Enforced in:** `observability-service/observability/internal/pubsubstore/pubsubstore.go` — `Register` (stream with both subject sets, `LimitsPolicy`, `StreamMaxAge`/`StreamMaxBytes`/`StreamDuplicates`, the `pubsub-messages` bucket, the durable `pubsub-store-projector` consumer), `tenantFromSubject`, `storeEnvelope`, `publishNotify`; wired in `observability/composition.go`'s `Startup`/`Stop`. Grants: `nats/bootstrap-operator.sh`'s `observability` user (the `PUBSUB`/`KV_pubsub-messages` analogues of every grant the trace store needed, `$JS.FC` and `$JS.API.DIRECT.GET` included up front rather than rediscovered live the way Phase 30i found them) and `accounts-service/auth/token.go`'s `MintAdminToken` (`notify._platform.kv.pubsub-messages.>` only — never `obs.pubsub.>`).
- **Test:** `observability-service/observability/internal/pubsubstore/pubsubstore_test.go` — own stream carrying both subject sets, bounded retention with an explicit `Duplicates` window, tenant derived from the remap subject (tenant *and* PLATFORM-local cases), redelivery deduplicated at both the stream and the KV write, a malformed envelope dropped without blocking the one behind it, the KV-change notify 43c subscribes to, and `Register`'s nil-safety. `accounts-service/auth/token_test.go` — the admin token's exhaustive `ConsistOf` grant list.

### BR-048 (Phase 43c, IMPLEMENTED 2026-08-25) — The Admin UI's Messages panel is a dedicated panel, names the originating tenant, and defaults to the `evt` family
...
- **Enforced in:** `frontend/admin/src/components/MessagesPanel.vue` — the panel itself (`familyOn` defaulting to `{ evt: true, notify: false }`, the `MAX_ROWS`/`truncated` cap, `paused`/`frozenOrder`/`displayedOrder`, the Tenant column and its click-to-filter, `SubjectPath` in the Subject column and detail head); `frontend/admin/src/nats/usePubsubFeed.js` — the `pubsub-messages` bootstrap-plus-`notify._platform.kv.pubsub-messages.>` feed carrying `{ tenant, span }` per record; `frontend/admin/src/App.vue` — the SYSTEM → NATS `pubsub` nav entry and its `data-testid="pubsub-view"` section.
- **Test:** `frontend/admin/src/components/MessagesPanel.spec.js` — reads the `pubsub-messages` bucket (not the trace bucket `RpcPanel` reads), the `evt`-only default and the family toggle, the per-row tenant from the remap, `SubjectPath` for subjects, pause freezing the visible rows while the feed keeps running underneath, the row cap and its note, the best-effort disclaimer, and a surfaced bootstrap failure. `frontend/admin/src/nats/usePubsubFeed.spec.js` — the feed's own bootstrap/live-upsert/reconnect behaviour.

### BR-049 (Phase 43a, IMPLEMENTED 2026-08-25; rewritten Phase 43d 2026-08-25) — Every `notify.*` message is observed, because every publisher goes through the seam — checked by a test

ADR-047's headline cost is that an uninstrumented publisher is silently invisible, and the ADR itself requires this be "a checked convention, not tribal knowledge."

**What this rule used to say, and why it changed.** Phase 43a had no `notify.*` seam, so BR-049 was a property of *places*: "every `notify.*` publish site is instrumented or explicitly excluded", enforced by an AST scan that flagged any function naming a `notify.*` literal and calling something beginning with `publish`. Phase 43d gave the family a seam (`shared/natsnotify`), and that premise died with it — a `Notifier` publishes and observes as one step, so a site cannot be half-wired, and "excluded" became a fact about how the `Notifier` was constructed rather than an entry in a table.

A rule phrased as "these seven places do X" has to be re-verified against every new publisher. Phrased as a property of the message it is true by construction — and what remains to check is not coverage but that **nothing bypasses the seam**.

**The two structural properties the scan asserts.** It still parses every non-test `.go` file under `demos/01-dictionary/backend/` with `go/ast`:

1. **`notify.*` subject literals appear only in subject-builder files** — a small allowlist, each entry carrying a reason. Everything else in the tree publishes through `natsnotify` and names no subject of its own.
2. **No function both names a `notify.*` subject and publishes it.** This is the bypass an allowlisted file could still commit: `subject := "notify." + ctx + …` a few lines above an `nc.Publish(subject, data)`. That shape is exactly what Phase 43d removed — the tokens are concatenated away and then guessed at again downstream.

Matching `publish` case-insensitively is retained from the original scan and is still deliberate: accounts-service's lifecycle notifies hand their subject to an unexported `publishAccountEvent`, and an exported-only match left them invisible to the first version of the scan — the precise failure mode the rule exists to catch.

**`shared/natsnotify` is deliberately absent from the allowlist.** It publishes every `notify.*` in the tree and knows the name of none of them. That split is the design: arities in this family run from four tokens to structurally unbounded, so there is no grammar a shared module could build against.

**Entries are also checked for staleness**, so an allowlist entry that no longer names any `notify.*` subject fails rather than lingering as folklore.
...
### BR-051 (Phase 48b/48c, IMPLEMENTED 2026-08-26) — A trace span's tenant is derived from its arrival subject, never from its envelope, and is stored per span
...
**Attribution belongs to the span, not to the trace.** 48b stored one tenant on the record and 48c corrected it: a stored element is `{tenant, span}`, mirroring `pubsubRecord{Tenant, Span}`. The correction was forced by the ordinary case, not an exotic one — the chain BR-054's harness drives (`api.{ctx}.organizations.…` → `rpc.{ctx}.refdata.…`) has two spans published inside the tenant's account and one inside PLATFORM, because `refdata-service` runs on `platform.creds`. A per-trace tenant would have to pick one of those and label the other falsely, on the one panel whose entire value is trustworthy provenance. Any trace-level tenant a caller wants is derived from the root span, not stored.

**The tenant stays *outside* the span object rather than merged into it.** The span is a document the observed account wrote; the tenant is a token the NATS server inserted. Keeping them in separate JSON fields is what makes the second unforgeable by the first — merging them would put a server-derived value into a publisher-controlled object, which is the spoofable model this rule exists to avoid, reintroduced at a different level.

**A record written before this rule needs no migration, but does need an explicit decoder.** Pre-48c records hold bare spans, and the default struct decoding would read one into an empty wrapper and **silently drop it**. `storedSpan.UnmarshalJSON` therefore probes for a `span` key: present means the wrapper, absent means a legacy bare span, kept with an empty tenant. Empty is a real value here — it renders as `unattributed` rather than guessing (BR-054) — and `TRACES` is `LimitsPolicy` capped at 1 h, so the legacy shape ages out on its own.

- **Enforced in:** `observability-service/observability/internal/tracestore/tracestore.go` — `PlatformSubjectWildcard`/`TenantSubjectWildcard` on `TRACES`, the removal of the consumer's `FilterSubject`, `tenantFromSubject`, the `storedSpan{Tenant, Span}` wrapper and its `UnmarshalJSON`, and `tenant` threaded through `appendSpan`. Read side: `admin/src/nats/useTraceFeed.js` — `normalizeSpans` flattens the wrapper to an `attributedTenant` field on each span for the panels.
- **Test:** `tracestore_test.go` — `TestRegisterCapturesBothSubjectSets` (verified red with the second subject removed), `TestTenantFromSubject` (both real shapes plus the near-misses that must *not* read as a tenant, including `monitor.*.pubsub.>`, which is a real subject on this server), `TestPlatformArrivalIsAttributedToPlatform`, `TestTenantIsDerivedFromSubjectNotEnvelope` — a span whose payload names a *different* tenant than its subject does, which is the spec the rule exists for and which an envelope-reading implementation would fail while passing every other spec in the file — `TestCrossAccountTraceKeepsBothAttributions`, and `TestLegacyBareSpansStillDecode` (verified red with the custom decoder removed, where the pre-48c span was dropped). Frontend: `useTraceFeed.spec.js` — `useTraceFeed span attribution`. Publishing side: BR-AC36. Panel side: BR-054.

### BR-052 (Phase 48b, RETIRED 2026-08-26 in Phase 48c) — ~~Within one `traceId`, the first tenant attributed wins~~

**Retired the day it landed, because it fires on legitimate traffic.** The rule read two tenants under one `traceId` as "never normal traffic" — either a `traceId` collision or an attempt to attach spans to another tenant's trace — and so froze the trace's tenant at the first writer, logging any later disagreement at warn level.

**It reasoned about tenant-vs-tenant and never considered tenant-vs-`_platform`.** The most ordinary cross-account request in this stack is exactly that: `organizations-service` runs tenant-scoped connections, `refdata-service` runs on `platform.creds`, so a single `api.*` call produces two `acme` spans and one `_platform` span under one `traceId`. Under BR-052 that logged a warn on every request, and the "disputed" attribution it was protecting was correct information about a genuine account crossing.

**Its premise was a per-trace tenant field, which BR-051 no longer has.** With attribution stored per span there is nothing to dispute: both values are kept, each against the span it belongs to, and the panel renders the crossing (BR-054). The concern the rule was reaching for — that a later write must not silently overwrite an earlier attribution — is satisfied structurally rather than by a guard, since no write ever touches another span's tenant.

Kept as a retired entry rather than deleted: the reasoning that produced it (first-writer-wins looks like the safe choice against last-write-wins) is reachable again from the same starting point, and the reason it is wrong is not visible from the code.

- **Enforced in:** nothing. The guard in `appendSpan` was removed in 48c.
- **Test:** none. `TestFirstTenantWinsOnMismatch` was replaced by `TestCrossAccountTraceKeepsBothAttributions`, which asserts the opposite: both attributions survive, and no warn is logged.

### BR-053 (Phase 48f/48g, IMPLEMENTED 2026-08-26 — bound 48f, write shape 48g) — `trace-request-reply` is a bounded visible window, sized from measurement, and one span is one idempotent write
...
**One span is one idempotent write.** `appendSpan` was a read-modify-write: read the whole trace, append, write it back. Two properties follow, one that held and one that held only by accident.
- Storing the same `spanId` twice must produce exactly one visible span. This held by dedup-on-merge.
- Two overlapping writers for one `traceId` must not lose a span. This held **only because there is a single projector instance**, and only because JetStream delivers to one consume callback at a time. It breaks the moment the projector is scaled or a redelivery overlaps — a lost span in a diagnostic tool is worse than a missing trace, because the waterfall still renders and looks complete.

The rule requires both **by construction**, and 48g built it that way: each span is keyed `_platform.trace.{traceId}.{spanId}` and written with a plain idempotent `Put`, and the trace is assembled at read time. Dedup then costs nothing — the same span overwrites its own key — and the read-modify-write window disappears rather than being narrowed. The read-modify-write was also O(n²) in span count for a single trace, which the per-span key removes as a side effect rather than as its purpose.

**The concurrency defect was real, and had to be provoked to be seen.** Driving it through `Register`'s consume callback cannot show it, because that callback is serialised — the projector is single-threaded by construction, which is exactly what was masking the bug. The spec calls the store function directly from 16 goroutines against one `traceId`; against the read-modify-write it stored **1** span of 16.

**`TRACES` sets its `Duplicates` window explicitly** (2 min, matching the server default it used to inherit), the way BR-047 made `PUBSUB` do — **and the span publish sets `Nats-Msg-Id`**, which is the half that makes the window mean anything. A `Duplicates` window with no message id on the message is inert: JetStream has nothing to compare. `natstrace`'s `finish` now publishes the span with `Nats-Msg-Id: {spanID}`, so a redelivered or double-published span is refused at the stream rather than deduplicated downstream. Both halves belong to one contract and neither is worth having alone.

**Known cost, accepted:** this changes the Admin UI's read path. `useTraceFeed.js` keyed entries by `traceId` and subscribed to `notify._platform.kv.trace-request-reply.trace.{traceId}.changed`; per-span keys mean a prefix-grouped read and a per-span notify subject, on the surface Phase 43e's reconnect/resync work had just stabilised. The narrower alternative — keep the merged key and make the write safe with a revision-CAS `Update` plus bounded retry — fixes the race but not the write amplification, and was considered and declined in Phase 48's decision 13.

Two things about that read path came out differently from the design, and are worth recording rather than rediscovering:

- **The feed's monotonic-merge guard was removed, not preserved.** It existed because a whole trace arrived in one entry, so a snapshot issued before a live notify but resolving after it would overwrite newer spans with older ones, and comparing span counts was the only defence. Per-span merging makes a stale read a no-op instead of a rollback, so the guard has nothing left to guard. The spec that pinned the old behaviour still passes unchanged, which is what makes the removal safe to assert rather than merely plausible.
- **The frontend reads both shapes, for exactly one `BucketMaxAge`.** Deploying the projector does not rewrite the merged records already in the bucket; they expire on their own within 15 minutes. `spansFromRecord` therefore accepts `{tenant, span}` and the pre-48g `{traceId, spans[]}` alike, which turns the deploy into a non-event rather than a blank panel. The Go-side legacy decoder was **deleted** in the same change — nothing on the write side reads a stored record any more, so keeping a decoder there would have been dead code pretending to be a compatibility shim.

- **Enforced in:** `tracestore.go` — the bucket config (`BucketMaxAge`, `BucketMaxBytes`, applied in `Register`), the stream's `Duplicates`, and `storeSpan`'s key shape and `Put`; `shared/natstrace/natstrace.go` — `finish` setting `Nats-Msg-Id`; `frontend/admin/src/nats/useTraceFeed.js` — `spansFromRecord`/`mergeSpans` and the prefix-grouped read.
- **Test:** `tracestore_test.go` — `TestRegisterCreatesBoundedBucket` asserts the TTL and `MaxBytes` the bucket was actually created with **and** that both stay strictly tighter than the stream's, which is the half a future "just bump it" edit would quietly violate. Verified red against the bare `KeyValueConfig{Bucket}` before the fix; every other spec in the file passes either way, because an unbounded bucket stores and reads back identically to a bounded one. `TestSpanIsStoredUnderItsOwnKey` asserts the per-span key **and** that the pre-48g merged key is not written; `TestRegisterDedupsRedeliveredSpan` covers the same `spanId` twice, now a property of the key rather than of a merge; `TestConcurrentSpanWritesLoseNothing` runs 16 concurrent writers against one `traceId` and was verified red (1 span of 16) against the read-modify-write; `TestStreamSetsDuplicatesExplicitly` pins the window. `shared/natstrace/natstrace_test.go` — *BR-053 — an `obs.trace.*` span carries `Nats-Msg-Id`*, mutation-verified. Read side: `useTraceFeed.spec.js` — `useTraceFeed per-span records (Phase 48g, BR-053)`, nine specs covering the join, per-span attribution surviving it, one `onUpsert` per trace rather than per span, live-merge and idempotency, a stale re-read adding nothing, both spanId-less gates, and a merged and a per-span record joining into one trace. The fifteen pre-48g specs above it pass unchanged, which is the back-compat proof.
...
### BR-054 (Phase 48h/48c/48i, PARTLY IMPLEMENTED 2026-08-26 — harness 48h, traces panel 48c, Messages panel 48i outstanding) — The Admin UI names the originating account on both observability paths, and a multi-span cross-account trace is what proves it
...
**`TraceWaterfall.vue`'s account gutter shows the real tenant name** (done, 48c), read from BR-051's stored per-span attribution, replacing the coarse PLATFORM/TENANT split it inferred from a hardcoded service-name list — and the comment documenting that split is removed rather than left stale. A span with no stored attribution renders as `unattributed`, in its own muted italic style: a pre-48c record must read as *unknown*, never as a guess, on a panel whose value is provenance. The gutter's two-colour vocabulary is deliberately kept — the bar's colour still says PLATFORM-or-tenant, because what the eye needs at a glance is the *crossing*; the name beside it says which tenant. The Messages panel needs no work at all, backend or frontend: the tenant has been on `pubsubRecord` since BR-AC34, and Phase 43c already surfaced it as the table's leading Tenant column plus a set of tenant filter chips — this rule's original "it is simply not surfaced" was stale when written, and 48i closed by verifying the live panel rather than by adding a second column. What that column shows is the *publisher's account*: an `evt.acme.refdata.vehicle-type.changed` row reads `_platform`, because refdata-service runs in PLATFORM and the `acme` in the subject is `{context}`, not the tenant.
...
**The harness drives the real stack, in both outcomes.** A genuine `api.*` call from a tenant account → `organizations-service` → `rpc.*` into `refdata-service`, run twice: once with a valid payload, where every span reports OK; once with a payload the callee rejects, where the error spans carry `StatusCode`/`StatusMessage` **and the root span still closes**. A parent left open on the error path is the failure this specifically guards — it renders as a truncated waterfall rather than as an error.

Two details of that chain were written into this rule from the design and corrected against the running stack when 48h was built:

- **The chain is `organizations-service`, not `shipping-service`.** Phase 32 removed shipping-service's five refdata relay routes, which left its `internal/refdataconsumer` with no live callers at all — the shipping → `rpc.*` → refdata chain this rule originally named no longer exists. `organizations-service` → `refdata-service` (BR-TP14's vehicle-type resolution, over the account's `refdata.item.get.v1` service import) is the only live cross-service, cross-account chain in the stack.
- **It drives the hop with the tenant's own credential, not a browser token.** `accounts-service`'s `connectInfo` returns 409 *"tenant has no signing key on record"* for the seeded pre-existing accounts (PLATFORM/ACME/GLOBEX), so no browser token can be minted for the tenant the compose stack actually runs. This costs the harness nothing it was testing: the account boundary — the thing being attributed — is identical either way.

**The error run's span statuses were not the obvious guess, and were recorded here rather than asserted around — then BR-055 settled them.** As built in 48h the chain read ERROR / **OK** / ERROR: the root `api.*` span, then the *caller-side* outbound hop (`refdata.item.get.v1`, the tenant account's local alias) closing OK because the request/reply round trip genuinely succeeded, then refdata's own inbound handler span (`rpc.{context}.refdata.item.get.v1`, the subject the server remapped that alias to) in ERROR from `browserrpc`'s `respondError` path. The open question this paragraph left — whether a delivered refusal should read as ERROR — was answered in 48j by BR-055, and answered without widening the status vocabulary: the middle hop holds a `Nats-Service-Error` header, so it is ERROR too, and the chain now reads red end to end. **The harness's transport-failure discriminator changed with it**: "at least one span closed OK" no longer separates a domain rejection from a dead responder, so `seed-traces` asserts BR-037's `rpc.retry_count` of `0` instead — a completed first-attempt round trip — alongside the existing requirement that an ERROR span come from `refdata`.
...
- **Test:** `observability-service/cmd/seed-traces` against the running compose stack, asserting on the stored trace: exactly one span parented to its own injected `traceparent`, every other span's parent present in the trace, spans from both services, and — on the error run — the root closed in ERROR, every ERROR span carrying a `statusMessage`, an ERROR span from `refdata` (so the failure is attributable *across* the boundary), and at least one OK span (so a transport failure cannot satisfy the check). Tenant attribution is asserted under an opt-in `-expect-tenant` flag (absent from pre-48b records): the named tenant must appear, `_platform` is the only other permitted value, and no span may be unattributed — deliberately *not* "every span carries the same tenant", which this chain correctly violates. Gutter: `TraceWaterfall.spec.js` — `TraceWaterfall account attribution`, three specs (the tenant is named rather than reading `TENANT`; a two-account trace counts as `2 accounts`; a pre-48c span renders `unattributed`), verified red against the old service-name heuristic.

---

### BR-055 (Phase 48j, IMPLEMENTED 2026-08-26) — A reply carrying `Nats-Service-Error` finishes its span as ERROR, whichever method the caller reached for

**A span that finishes holding micro's `Nats-Service-Error` header is not OK.** `natstrace.End` reads the header off the reply headers it is handed; when present, the span finishes `ERROR` with the header's value as its `statusMessage` (and, for a pre-Phase-28 consumer, as the envelope's `error`). No numeric code is consulted: a `404` is as much a refusal as a `500`, and `Nats-Service-Error-Code` exists to tell a *caller* which refusal it got, not to tell an observer whether one happened.

**This is a defect fix, and the defect was one panel disagreeing with itself.** `End` hardcoded `finish("OK", …)` and only `Fail` ever wrote `ERROR`, while every caller in the repo calls `End` on any reply that arrives and `Fail` only when the transport gave up — see `refdataclient.Client`, whose retry loop is explicit about it. So an `organizations → refdata` lookup for a missing item drew a blue bar in `TraceWaterfall.vue` while, three rows below, the same panel's detail pane rendered that span's own `Nats-Service-Error` in red via its `errv` class. The header was captured, stored, and displayed as an error — and never once consulted when deciding the status.

**The decision belongs in `natstrace.End`, not at the call sites and not in the UI.** Every outbound span in the repo agrees at once; the stored KV record is right rather than repaired at render time; and it matches micro's own convention, in which the header *is* the error channel. Fixing it in `TraceWaterfall.vue` would have left the record wrong for every other consumer of `obs.trace.*`; fixing it in `refdataclient` would have left the next caller to rediscover it.

**Two boundaries the rule draws deliberately:**

- **Only the reply's headers are read, never `sp.reqHeaders`.** A handler whose *inbound* request carried the header has not itself failed — a service relaying a failed upstream call's headers would otherwise report its own successful work as ERROR.
- **An explicit `Fail(err, …)` outranks a header.** The caller diagnosed something the header did not describe (a timeout, an exhausted retry loop), and that diagnosis is the more specific one.

**The body-only refusal shape is out of scope, by choice.** `refdata-service` answers some paths with an ordinary reply carrying `{"error":…,"notFound":true}` and no header at all (see `seed-transporters`' `refdataRequest`, which has to check both). Those spans stay OK. Reading them would mean `natstrace` parsing payloads to decide a status — a much wider blast radius for a shape that is itself worth converging on the header convention instead.

- **Enforced in:** `shared/natstrace/natstrace.go` — `Span.End`, with `ServiceErrorHeader`/`ServiceErrorCodeHeader` now named constants beside `RequestorHeader`/`ResponderHeader`.
- **Test:** `shared/natstrace/natstrace_test.go` — `BR-055 — a reply carrying Nats-Service-Error finishes the span as ERROR`, four specs: the ERROR case (verified red against the old `End`), a clean reply staying OK, an inbound-header request not turning its own span red, and `Fail`'s message surviving. Call-site net: `organizations-service/.../refdataclient/client_test.go` — `BR-055 — a refused reply is an ERROR span, not an OK one`, driving a real micro-shaped refusal through the client's own `End` path and asserting `rpc.retry_count` is `0`, which is what made the old behaviour look defensible (also verified red). Panel side: `TraceWaterfall.spec.js` — `TraceWaterfall error rendering`, four specs over the same refused three-span chain (every bar draws as an error including the caller-side hop; the trace tile and header read failed; the `errors` filter keeps it and drops a clean trace; the response pane styles `Nats-Service-Error` and only that header as an error) — all four verified red against a component mutated back to the pre-48j vocabulary, under which the sixteen pre-existing specs stayed green, which is the measure of how little the render side was pinned. Live chain: `seed-traces`' error run, whose recorded shape and transport-failure discriminator changed with this rule — see BR-054. Verified against the running compose stack 2026-08-26: the ERROR run reads ERROR/ERROR/ERROR across `api.acme.organizations.fleet-asset.add.v1` → `refdata.item.get.v1` → `rpc.acme.refdata.item.get.v1`, the OK run stays all-OK, and `-expect-tenant acme` passes.
...
### BR-058 (IMPLEMENTED 2026-08-26) — The Connections panel names the *credential* a connection authenticated with, decoded from a JWT that is never forwarded to the browser

**A presentation rule with a backend mechanism, scoped to the Admin UI only** — same family as BR-028, and it shares that rule's Connections data path (`GET /api/nats/connections`, `/connz` primary). BR-028 answers "which account is this connection in"; this rule answers the question BR-028 cannot: **which credential did it present**. The two are independent — several credentials can authenticate into one account, and one credential can be presented by several connections — which is why this is its own field and its own rule rather than more of BR-028.

**The name comes from the user JWT's `name` claim.** `/connz` already returns the connection's full signed user JWT, and the proxy was already requesting it (`?auth=true`) and discarding it. `credentialName` splits the token, base64url-decodes the payload (`RawURLEncoding` — NATS JWTs are unpadded), and reads `name`.

**The raw JWT is never forwarded to the browser.** It is decoded for that one claim and then dropped. Not because it is secret — the seed is the secret, not the token — but because it is ~1.5KB per row and embeds the credential's entire pub/sub permission grid, neither of which a connections table has any use for. This is the half of the rule with a security consequence, and it is the half that is asserted directly rather than inferred: the test greps the serialized response body for the token and requires it absent.

**The signature is deliberately not verified.** Nothing here authorizes on the claim. The NATS server already vouched for it by admitting the connection; this only reads a label the server itself accepted. Verifying would require the account's key material in `observability-service` — real trust surface, bought for a check that would prove nothing the connection's existence hasn't already proven.

**Fallbacks are ordered and explicit.** A connection that authenticated without a user JWT (`jwt` empty) falls back to `name_tag` — the *account* JWT's own name, a different kind of value, which is why it is a fallback and not the primary. With neither, the cell renders an em-dash rather than an empty cell.

**The user NKey rides along, but never gets a column.** `authorized_user` is the only stable identity for a credential — two distinct users can carry the same `name` — so it is exposed as `userKey` and shown beneath the credential name in the cell and in the detail pane's `User NKey` row. It stays out of the table because it is a debugging detail rather than something an operator scans a table for, and because the ephemeral browser credentials mint a fresh NKey per session (`auth/token.go`'s `mintUserToken`), so for those rows the value legitimately changes on every reconnect — a column of values that rotate under you is worse than no column.

**A credential whose name differs from the connection's own name renders amber, as a signal — not an error.** It means one of two things, both worth seeing: a deliberately shared credential, or the wrong `.creds` file mounted. This is the practical payoff of the credential-naming convention in `ARCHITECTURE-ACCOUNTS.md` § "Credential naming" (a dedicated credential is named for its holder, spelled as that process's `nats.Name()`), and it is why the amber is not red. Note that the convention itself is **not** part of this rule and is enforced by nothing today — `nsc` accepts any string. On the stack as it stands most rows are amber; that is the convention not yet applied, which this column now makes visible.

- **Enforced in:** `observability-service/observability/internal/rest/nats_connections.go` — `credentialName`, the `natsConnection.User`/`UserKey` response fields, the `connzConnection.JWT`/`AuthorizedUser`/`NameTag` wire fields, and the `name_tag` fallback in the mapping loop; `frontend/admin/src/components/ConnectionsPanel.vue` — the `Credential` column, `credentialDiverges`, the elided `<NKey>` beneath the credential name, the detail pane's `Credential` + `User NKey` rows, and the `.cred.diverged` treatment
- **Test:** `TestListNatsConnectionsExposesCredentialNameAndKey` (`nats_connections_test.go`) — two connections presenting one JWT both report the same `user` and `userKey`, the shared-credential case the rule exists to make visible, *and* asserts the raw token appears nowhere in the response body. `TestListNatsConnectionsFallsBackToNameTagWithoutUserJWT` — an empty `jwt` falls back to `name_tag`. `TestCredentialNameIgnoresMalformedTokens` — a table of seven malformed inputs (empty, wrong segment count, bad base64, non-JSON payload, no `name` claim) each yield `""` rather than a panic or a garbage label, since the input is attacker-adjacent in the sense that it is whatever the server happened to accept. `ConnectionsPanel.spec.js` — the credential renders, `diverged` is applied on mismatch and withheld on match, the em-dash renders when the backend supplied nothing, the NKey renders elided in the cell with no `title` (BR-061), and both detail-pane rows render

> **2026-08-27 amendment — the user NKey moved off the `title` and into the cell, elided.** The paragraph above originally read "shown on the cell's `title`". That was a tooltip carrying all 56 characters, which is the full render BR-061 forbids — merely delayed by one hover, and unreachable entirely on a touch device or by keyboard. The key now renders in the cell itself as `[FIRST5...LAST5]` beneath the credential name, per BR-061. **This is a relocation, not a deletion:** the fact the paragraph argues for — that `authorized_user` is the only stable identity for a credential — is more visible now, not less, since it no longer requires knowing a tooltip is there. The clause about it not getting a *column* still holds: it shares the Credential cell rather than claiming one of its own, for the reason stated above (a value that rotates per reconnect makes a poor column to scan). Two further tooltips on the same fact went with it — `UsersPanel.vue`'s name cell and its Account cell — the second of which was not catalogued until BR-061's absence assertion found it.

### BR-059 (IMPLEMENTED 2026-08-27) — A provisioned tenant has all three KV buckets, so "nothing here yet" reads as an empty bucket rather than a missing one

**Every other operation on `kvstore.Store` provisions its bucket lazily, and that stays true — this rule adds one eager call at tenant-provisioning time, it does not change how the Store behaves elsewhere.** Lazy creation is correct for the write path: the first `Put` creates the bucket, and no read can precede a write for data that only that write produces.

**What lazy provisioning cannot express is the difference between "empty" and "absent".** From outside the process those two states are indistinguishable — a bucket that has simply never been written does not exist. This is not a distinction the domain cares about, but it is one the Admin UI's cross-account reader is forced to make: `GET /api/kv/buckets/{account}/{bucket}/entries` (observability-service, `rest/kv.go`) answers `jetstream.ErrBucketNotFound` with a 400 `unknown bucket: {name}`. A tenant with no ships yet therefore made the Admin UI log a red `unknown bucket: ships` on every page load — an error reporting the correct absence of a bucket, in answer to a question whose honest answer was "no ships".

**Which of the three buckets existed was decided by traffic, not by the tenant.** `container` and `meta` were routinely present and `ships` was not, because provisioning order fell out of whichever query a given stack happened to run first. That is the actual defect: not that a bucket was missing, but that *which* were missing was arbitrary and varied between two stacks holding identical tenant state. An invariant that holds only sometimes is harder to reason about than one that never holds.

**So the three buckets become an invariant of a provisioned tenant** — established in `buildTenantResources` alongside the `SHIPPING` stream, which is already created eagerly there for the same reason. After provisioning, all three exist and all three are empty; a reader distinguishing empty from absent now gets a truthful answer without the domain having to care that the distinction exists.

**`Ensure` is deliberately not folded into `New`.** Most of this package's call sites are tests that want the lazy behaviour and have no `ctx` to give. Making construction provision would push a context parameter through all of them to buy an eagerness only the tenant-provisioning path needs. It is also deliberately not a write: creating the bucket with a placeholder entry would make it exist at the cost of putting a value in it that no reader expects and every reader must then filter out.

**This is a provisioning rule, not a read-path rule.** It does not make KV a list source and does not change BR-038 — the ship list still comes from the Postgres projection. It only guarantees that the cache's container exists from the moment the tenant does.

- **Enforced in:** `internal/kvstore/kv.go` — `Store.Ensure`, which calls the same `bucket(ctx)` accessor every other operation goes through, so provisioning stays in one place; `dictionary/internal/rest/tenant.go` — `buildTenantResources` calls it for `kvShips`/`kvContainers`/`kvMeta` in the loop that already enables notify on each, failing tenant provisioning outright if any bucket cannot be created
- **Test:** `internal/kvstore/kv_test.go` — `Ensure() creates the bucket without writing anything, so a never-written bucket still exists` asserts the precondition first (a freshly `New`'d Store has *not* provisioned its bucket, so the spec is proving `Ensure` did it and not something else), then that the bucket exists after the call and that `status.Values()` is zero, pinning "creates" as distinct from "writes a placeholder". `Ensure() is idempotent — a second call on an existing bucket succeeds` covers the re-provisioning path, since tenant provisioning re-runs for an already-provisioned tenant


### BR-060 (Phase 50c, IMPLEMENTED 2026-08-27) — The Users panel lists the registry, joins `/connz` for live counts, and derives Health from the JWT alone

**A presentation rule with two sources, and the whole rule is about keeping them apart.** The **roster** — who exists — comes from the registry accounts-service writes at mint time (BR-AC38), read over `api._platform.accounts.user.list.v1` (BR-AC40). The **live counts** — connections and subscriptions — come from `/connz` via the same `GET /api/nats/connections` path BR-028/BR-058 already use. Neither is derivable from the other: the resolver stores no users, so `/connz` knows only who is connected *right now*, and the registry knows nothing about connections.

**The join key is the user NKey, never the name** — BR-058's reasoning applies unchanged here, and applies harder: two distinct users can share a `name`, and a browser session mints a fresh NKey per session, so several rows named `admin-app` are genuinely several users.

**The join happens in the browser, not in accounts-service.** accounts-service has no HTTP-monitor client and should not acquire one to satisfy a table: it owns the registry, observability-service owns `/connz`, and joining them in the one place that already holds both keeps that ownership intact. The visible cost is that the two halves fail independently, which is the next clause.

**A `/connz` failure empties the counts but must not empty the roster.** The registry is the half that answers "who exists"; the connection counts are an overlay on it. A row therefore renders with `0` conns and `0` subs when `/connz` is unavailable, and the rows themselves stay. The converse — a registry failure — has no such fallback, because there is nothing left to render.

**`/connz`'s `limit` is a page size, not a capacity, so a paged response is surfaced rather than silently under-counting.** Same reading as the Connections panel. When the response is partial the counts column carries an amber `partial · /connz paged` note: an under-count that says so is usable, one that doesn't is a lie in a column an operator reads to decide whether a credential is in use.

**Health is derived from the JWT and from `status`, never from a connection.** `pending` (the BR-AC38 stuck-mint state) → `pending`; no expiry → `valid`; past expiry → `expired`; within one hour → `expiring`; otherwise `valid`. The bound is one hour because the browser session token's own lifetime is on that order, so anything longer would mark every session row permanently.

> **2026-08-27 amendment — the single hour-wide `expiring` band is split in two, and expired rows are hidden by default.** The one-hour bound above is what this replaces, and it failed for exactly the reason it was chosen: the browser session tokens whose lifetime it was sized to are the rows that then sat in the warning state permanently, so an amber row stopped carrying information. The bands are now, tested **narrowest-first because they nest** — testing the wide band first would swallow the narrow one:
>
> | Band | Colour | Condition |
> | --- | --- | --- |
> | `valid` | green `#27c07f` | no `exp` claim at all, or more than 15m remaining |
> | `scheduled` | yellow `#f0b429` | 15m or less remaining — "this will renew shortly" |
> | `expiring` | orange `#f5820b` | 2m or less remaining — "this is going now" |
> | `expired` | red `#e5484d` | `exp` has passed |
>
> `scheduled` shares `pending`'s yellow deliberately: both mean "in flight, nothing to do". `revoked` keeps `expired`'s red and still outranks every band above, per the amendment below.
>
> **The summary tile counts `scheduled` + `expiring` as one number** and is labelled `Expiring < 15m`. Splitting it would produce two numbers neither of which is worth a glance; the tile answers "is anything about to go away".
>
> **A second checkbox, `hide idle`, also defaults to ON — and it is the one filter here that can be *wrong* rather than merely selective.** It hides rows with no live connections, which on this stack is most of the roster. Unlike `hide expired`, which reads the row's own record, this one reads the `/connz` join, and a join can be missing or short. So it **suspends itself** — visibly, in amber, with the checkbox still checked and the word `suspended` where its count would be — in two cases:
>
> - **`/connz` failed.** Every row then reads `0`, and the filter would empty the whole roster. That would break this rule's own "a `/connz` failure empties the counts but must not empty the roster" clause outright.
> - **`/connz` answered one page.** A connection on an unread page counts as zero here, so a genuinely-connected row would be hidden with nothing on screen saying so — a louder version of the exact lie the amber paged note exists to prevent.
>
> Suspended means suspended, not inverted: the box does not silently uncheck itself, because that would discard what the operator asked for and then not say so.
>
> **"Idle" means quiet, not merely zero — and that exemption is what keeps this checkbox from cancelling out BR-062.** A credential being refused every few seconds also reads `0` connections, and it is the single row an operator most needs to see; hiding it would leave the Last outcome column with nothing left to report. So **a row whose last retained outcome was a refusal is never idle**, however few connections it has. A clean `Client Closed` is idle; an `Authentication Failure` is not.
>
> **The two checkboxes compose and their counts do not double-count.** A row that is both expired and idle is attributed to `hide expired` alone *when both are on*, so the two held-back counts always sum to the number of rows actually missing from the table. Since Phase 52 flipped `hide expired` off by default, the same row is attributed to `hide idle` in the default view — which is the correct reading of why it is missing, not an inconsistency.
>
> **A `hide expired` checkbox defaults to OFF (amended by Phase 52, 2026-08-27 — it shipped ON earlier the same day).** It defaulted ON because nothing deleted a session row: an expired session accumulated for as long as the stack had been up — 44 of 56 rows, measured — and the unfiltered roster was mostly rubble with the handful of credentials that mattered buried in it. Phase 52's reaper (BR-AC44) removes the rubble at the source, bounding the expired set at one 24h retention window rather than at the stack's whole uptime. At that size the expired rows stop being noise: a session that died inside the last day is the row an operator is reading when they ask why a tab dropped, so hiding it by default would be hiding the useful part. **The checkbox stays, because a window is a policy and 24h is not everyone's — and because it is a filter, not a deletion, a distinction that remains load-bearing:** the summary tiles above the table count the whole population regardless of the checkbox, and the checkbox itself carries the count of what it is holding back — a hidden row that says nothing about being hidden is the same lie as the under-count the paging note exists to prevent. It composes with the kind and text filters rather than overriding them, and it **never hides a `revoked` row**: revoked is not expired, and it is the state an operator most needs to see.

> **Reconciliation with the proposed rule (recorded because the shipped rule differs).** The version approved at the design gate said a `session` row is *observed-only* — "absent when not connected, and never presented as if the roster were complete" — and listed Health's states as valid / expiring / expired / **unused**. Two parts of that no longer hold once 50a shipped. **(a)** Sessions *are* enumerable now that the registry records them at mint, so a session row no longer depends on being connected, and the completeness caveat it was carrying moves off the roster and onto the `/connz` join, where the paging note above now carries it. **(b)** `unused` — "never connected" — is a fact only `/connz` can supply, which the rule's own second clause forbids Health from reading. Keeping both would have made the rule self-contradictory, so `unused` is dropped and `pending` takes its place: it is a JWT/registry fact, and it is the state an operator actually needs flagged.

> **2026-08-27 amendment — Health gained a `revoked` state (Phase 51b, BR-AC43), and it outranks the rest.** Once a credential's key is in its account's revocation list, what its `exp` claim says and whether its mint completed have both stopped mattering, so `revoked` is tested first and `pending` / `expiring` / `expired` never reach it. **This does not reopen the rule's second clause.** `revoked` is read from the registry row's `revokedAt` — a fact about the credential's own record, the same kind of fact `status` and `expiresAt` already are — and not from `/connz`. The distinction the clause protects is *record vs. connection*, not *static vs. changing*.

**The claims table shows what the server will enforce, not what was signed** — this is BR-AC41 rendered. Effective permissions come first; grants the scope discarded are struck through and dimmed beneath them, under a stated override banner, and are present only when the service reported that a scope discarded them. An unresolvable scope prints its stated reason verbatim and renders no claims table at all, rather than an empty one that would read as "no permissions".

**Absent and `-1` both mean unlimited.** `jwt.UserPermissionLimits` marks `subs`/`data`/`payload` `omitempty`, so a missing field and an explicit `-1` are the same fact and both render `unlimited`. `0` is never a meaningful value for these.

- **Enforced in:** `frontend/admin/src/components/UsersPanel.vue` — `healthOf` and the `SCHEDULED_MS`/`EXPIRING_MS` bounds, `EXPIRY_SOON`/`expiringCount`, `hideExpired`/`expiredCount`, `hideIdle`/`idleCount`/`idleFilterActive`/`isQuietlyIdle` and the `connsAvailable` flag that separates "nothing is connected" from "we do not know what is connected", the two `rowMatches` clauses that apply them, `connsByUserKey`/`liveFor`, `connsPartial`, `permRows`/`effectiveRows`/`discardedRows`, `limitLabel`/`byteLabel`, and the `.claims tr.struck` / `.unresolved` / `.override` treatments; `frontend/admin/src/api.js` — `listNatsUsers`/`getNatsUser` over the PLATFORM browser connection; `frontend/admin/src/App.vue` — the `Identity` nav group and the `users` view
- **Test:** `UsersPanel.spec.js` — the roster lists both kinds credentials-first; an unconnected credential is `valid` with `0`/`0` (the case that would regress if Health ever read `/connz`); the four expiry bands are derived from expiry against a frozen clock, each asserted at its boundary (exactly 15m, a second inside 15m, exactly 2m, a second inside 2m, the expiry instant itself) so the nesting order cannot silently invert; the tile counts both live bands; `hide expired` is OFF at mount so expired rows are visible, states its held-back count once switched on, restores and re-hides on toggle, leaves the summary counts alone, keeps a revoked row visible, and composes with the kind filter; `hide idle` is on at mount and leaves only the two connected rows, counts the idle-and-expired row once rather than twice once both are on, keeps an idle row whose last outcome was `Authentication Failure` visible and uncounted while still hiding one whose last outcome was `Client Closed`, suspends itself with the roster intact and the box still checked when `/connz` rejects and again when `/connz` answers one page of many, and composes with `hide expired`; a `pending` row shows as pending; the NKey join sums two connections into `2`/`5`; the roster survives a `/connz` rejection; the paged note appears only when the response is partial; kind and text filters; a drill-in with nothing discarded renders no strike-through; the scoped case renders the override banner, exactly two struck rows, an exact ordered list of effective subjects and effects, `Max subs 50`, `1 MiB`, `WEBSOCKET`, `Data unlimited`; an unresolvable scope repeats its reason verbatim with no claims table; a failed claims read surfaces on the row; a bearer credential is counted and flagged
### BR-061 (Phase 50e, IMPLEMENTED 2026-08-27) — An NKey is never rendered in full in the Admin UI

**A presentation rule that applies to every panel, present and future, which is why it is its own rule rather than a clause inside BR-058.** An NKey is 56 characters. Nothing an operator does with one on screen requires all 56: they are *recognising* which key a row carries, not verifying it. Verification is a `nsc`/clipboard job, and the clipboard is where this rule sends it.

**The rendered form is `[FIRST5...LAST5]`, with no spaces around the glyph.** The leading character is itself meaningful — `A` account, `U` user, `O` operator — so the first five characters carry a type as well as an identity, and the last five make two keys of the same type distinguishable at a glance. The glyph is the literal three periods, **not** `…`: the elision is a claim about characters removed, and a single-glyph ellipsis renders at a width that reads like punctuation rather than omission at this size. Padding the glyph out to ` ... ` was tried and reverted — it stretched a ten-character token wide enough to compete with the name beside it, for no gain in legibility. The brackets mark the whole token as one deliberately-shortened value rather than a string that happened to get cut off.

**It sits beside the value it identifies, on the same line — never stacked under it.** The pair is one fact in two halves and reads as one when it stays on one line; stacking made every row of a dense table two lines tall to carry one short token. The columns that carry a pair are sized to hold both halves and are marked `pair-cell` so the flow cannot wrap them back apart, which costs some width the surrounding columns give up.

**Dimmer and smaller than the value beside it.** `#8a9099` — a step below `--p-text-muted-color`, which sat close enough to the name that the two competed — at 9px, a second step down from the 10px this first shipped at, because at 10px the token still read as a peer of the name rather than as its subscript. Size and tone were settled separately and deliberately: a run that took the colour down to `#6b7079` alongside the 9px was reverted the same day, so `#8a9099` is the floor. It is around 5:1 on the panel background, which the size does not change — the token is still compared character against character, and a key nobody can read identifies nothing. The dimming is by colour rather than an opacity on the token, which would have taken the ten significant characters down with the brackets.

**The rule is enforced as an *absence*, and that is the part that has teeth.** Before this rule the same fact was drawn four different ways across four panels — `ConnectionsPanel`'s `shortAccount()`, `AccountsPanel`'s `slice(0, 12)…`, `AccountsOverviewPanel`'s `shortAccount()`/`rawFallback()`, and `UsersPanel`'s bare `:title` — three of which hung the full 56 characters off a `title` tooltip. **A full render one hover deep is the full render the rule forbids, merely delayed** — and it is worse than a visible one, because it is unreachable by keyboard and on touch while still being in the DOM. So the specs assert what is *not* there: the panel's rendered HTML does not contain the key, and no `[title]` attribute carries it. A future panel that reintroduces a tooltip fails without anyone remembering this rule exists.

**Copy is a detail-pane affordance only.** The rule is that a key is never *shown* in full, not that it can never be *obtained*: a pane is where an operator goes to fetch a key for `nsc`, and removing the full render without giving them any way to get it would make the pane worse at the job it exists for. **The clipboard may carry all 56 characters; the screen may not.** Table cells pass no `copyable` — a column exists to let you recognise a row, not to extract a key from one, and a control on every row would be noise in a dense table. A denied or unavailable clipboard fails silently: there is no fallback that does not put the full key back on screen.

**One helper, one component, one enforcement point.** `elideNKey()` in `format.js` is the only place the format is decided; `NKey.vue` is the only way it is drawn, and it owns the size and tone, and it exposes no prop that expands it. A panel that wants an NKey on screen mounts that component.

**`SharingPanel.vue`'s account field is explicitly out of scope** — its input is a *name or* a key depending on how the share was created, and eliding a name would corrupt a value the operator typed. Routing it through the helper would make the rule wrong there rather than more consistent.

- **Enforced in:** `frontend/admin/src/format.js` — `elideNKey`, `NKEY_GLYPH`, the `NKEY_MIN_LENGTH` floor below which a key is returned unchanged (the two halves plus the glyph would be longer than the key itself, so the "shortened" form would be the lie); `frontend/admin/src/components/NKey.vue` — the component, its `copyable`/`empty` props, and the 9px/`#8a9099` treatment; the `.cell-nkey` / `pair-cell` pairing and the column widths that keep a pair on one line in `ConnectionsPanel.vue` and `UsersPanel.vue`; call sites in `ConnectionsPanel.vue` (Account and Credential cells, both detail-pane NKey rows), `UsersPanel.vue` (name cell, Account cell, `Account NKey`/`User NKey`/`Issuer` pane rows), `AccountsPanel.vue` (Public Key column) and `AccountsOverviewPanel.vue` (`displayName`'s fallback — which still returns a string, because the search filter reads it, and searching a value the UI never shows would be a filter that matches invisibly)
- **Test:** `format.spec.js` — the format, `NKEY_GLYPH === '...'` with no `…` anywhere, that the middle never reaches the screen (stripping punctuation from the output leaves exactly ten characters), the too-short passthrough, and empty/null/undefined → `''`. `ConnectionsPanel.spec.js` — the elided account fallback; **never puts a full NKey on a table cell, on screen or in a tooltip** (the rendered HTML contains neither key and the table carries no `[title]` at all); no copy control in a table cell; the detail pane's copy control writes the *full* key to a stubbed clipboard. `UsersPanel.spec.js` — three specs against locally-defined 56-character keys rather than the file's own short fixtures, which sit below the elision floor and would pass the rule by accident rather than by obeying it. `AccountsPanel.spec.js` — a file that exists for this rule alone, the panel's actual behaviour being covered by accounts-service's Go specs. `AccountsOverviewPanel.spec.js` — the unresolved-account row renders elided and carries no `title`

### BR-062 (Phase 51a, IMPLEMENTED 2026-08-27) — A credential's last connection *outcome* is read from `/connz`'s closed ring, and is a reason rather than a timestamp

**The question a Users panel cannot answer from the live list.** BR-060 joins `/connz` for live counts, which tells an operator whether a credential is connected *now*. It cannot distinguish "this credential is idle" from "this credential is being refused every few seconds", and those are the two states an operator most needs to tell apart. `/connz?state=closed` carries the answer: the server keeps a ring of recently-closed connections, each with a `reason` and a `stop`.

**The reason is the signal; the timestamp is supporting detail.** A row's outcome is `Authentication Failure`, `Client Closed`, `Server Shutdown` and so on — a fact about *why* access ended, which is what changes an operator's next action. A last-seen timestamp was the alternative framing and was rejected: "last seen 4 minutes ago" reads as reassurance whether the connection closed cleanly or was refused.

**Reason for every kind; `stop` detail only for credentials.** The refusal is worth showing on a session row too — a session being refused is the same bug as a credential being refused, and suppressing it would hide it exactly where a browser session's rapid reconnects make it most visible. The *timestamp* is not: a session's last stop is a fact about a 15-minute mint that was always going to end, so a per-session "last seen" would fill the column with noise. That split is deliberate and is not the same argument applied twice — one is about a refusal, the other about a clock.

**A separate endpoint, not another field on `/api/nats/connections`.** `GET /api/nats/connections/closed` is its own route. The closed ring is far larger than the live list and only the Users panel reads it; folding it into the live response would make every Connections-panel refresh pay for data it never draws. It asks for `auth=true` (it needs `authorized_user` and the JWT to name the credential) and deliberately **not** `subs=true` — subscription detail on a closed connection is meaningless.

**The join key is the user NKey, exactly as in BR-058 and BR-060.** `authorized_user` is the only stable identity; the `name` is not unique. The raw user JWT is decoded server-side to recover the credential's `name` claim and is then **dropped**, never forwarded to the browser — the same treatment BR-058 already gives the live list.

**Rows are returned most-recently-stopped first**, so the browser's per-NKey reduction is a first-match and never has to sort. The reduction itself happens in the browser, because BR-060 put the users×connz join there on purpose.

**Absence is "outside the retained window", not "never disconnected".** The ring is bounded and the response is paged like the live one. A credential with no closed row is presented as having no *retained* outcome rather than a clean history, and a partial response carries the same amber paged note BR-060 uses — for the same reason: an under-count that says so is usable, one that doesn't is a lie.

**The outcome is only shown when the credential has no live connections.** A connected credential's last closed connection is history; its live count is the answer.

**The value set is nats-server's own `ClosedState`, passed through verbatim.** The panel never maps, normalises or shortens a reason — it only decides whether to colour one. The strings come from `ClosedState.String()` in `server/monitor.go`; the full set as of nats-server v2.14.5, grouped by what an operator does about each:

| Group | Values | What it means here |
| --- | --- | --- |
| **Refusal** — coloured, the case the column exists for | `Authentication Failure`, `Authentication Expired`, `Authentication Timeout`, `Credentials Revoked` | The server rejected the credential. `Credentials Revoked` is BR-AC43 landing (nats-server's `Revocation` state); `Authentication Expired` is a JWT `exp` reached mid-connection, which is normal on a session row and a misconfiguration on a credential row. |
| **Clean** | `Client Closed`, `Server Shutdown` | Either end hung up deliberately. The resting state of a healthy row. |
| **Transport** | `Read Error`, `Write Error`, `Stale Connection`, `TLS Handshake Failure`, `Incorrect Port`, `Proxy Not Trusted`, `Proxy Required` | The link broke rather than being refused. `Read Error` is what a restarted server leaves behind on its clients. |
| **Client misbehaviour** | `Slow Consumer (Pending Bytes)`, `Slow Consumer (Write Deadline)`, `Parse Error`, `Protocol Violation`, `Bad Client Protocol Version`, `Message Header Violation`, `No Responders Requires Headers`, `Minimum Version Required` | The client broke the contract. A slow consumer is the one worth alerting on. |
| **Limit** | `Maximum Connections Exceeded`, `Maximum Account Connections Exceeded`, `Maximum Message Payload Exceeded`, `Maximum Control Line Exceeded`, `Maximum Subscriptions Exceeded` | An account or server limit bit — the tenant-isolation boundary doing its job, or sized wrong. |
| **Administrative** | `Kicked`, `Missing Account` | `Kicked` is an operator ejecting the connection; `Missing Account` means the account JWT is gone from the resolver. |
| **Cluster topology** — never on a client row | `Duplicate Route`, `Route Removed`, `Wrong Gateway`, `Cluster Name Conflict`, `Cluster Names Identical`, `Duplicate Remote LeafNode Connection`, `Duplicate Client ID`, `Duplicate Server Name`, `Internal Client` | Route/gateway/leafnode plumbing. This POC is single-server, so these do not appear. |
| **Fallback** | `Unknown State` | nats-server's own default arm for a state its `String()` does not cover. |

**A reason is coloured if and only if it matches `/Authentication|Authorization|Revoked/i` *and does not match* `/Expired/i`** — which selects exactly the Refusal group above and nothing else. The test is on the string rather than on a mapped enum on purpose: **an unrecognised reason renders plainly rather than being mapped to a state the server did not report**, so a nats-server upgrade that adds a `ClosedState` shows the new string uncoloured instead of being silently mis-grouped. `Authorization` is in the pattern defensively; no current `ClosedState` produces it.

> **2026-08-27 amendment (Phase 52) — the `Expired` exclusion, and why it has to come first.** `Authentication Expired` matches the refusal pattern textually but is not a refusal: it is the server's word for a session whose own TTL ran out, which is the ordinary and expected end of every browser session in this stack and, by volume, the commonest reason in the closed ring (35 of 59 in the Phase 51 measurement). Grouping it with `Authentication Failure` was wrong twice over. It coloured a routine expiry as a problem — cosmetic. And because `isQuietlyIdle` treats *a row whose last outcome was a refusal* as never-idle, precisely so the rows this column exists for survive `hide idle`, it also made `hide idle` structurally unable to hide the single largest class of dead rows. That second effect stayed invisible while `hide expired` defaulted ON and claimed those rows first; flipping that default (BR-AC44) exposed it immediately as **41 rows visible on a roster with 8 live connections**. The lesson worth keeping: two filters that overlap can hide each other's bugs, and changing which one runs first is a real test of both.

- **Enforced in:** `backend/observability-service/observability/internal/rest/nats_connections_closed.go` — the route, the `state=closed&auth=true` query, the JWT-name/`name_tag` fallback, the stop-descending sort and the paging envelope; `.../rest/nats_connections.go` — `Stop` and `Reason` on `connzConnection`, populated only by `?state=closed`; `.../rest/handlers.go` — the route registration (BR-040's allowlist, extended); `frontend/admin/src/api.js` — `getNatsClosedConnections`; `frontend/admin/src/components/UsersPanel.vue` — the outcome cell and its live-count precedence.
- **Test:** `nats_connections_closed_test.go` — the request asks for the closed ring with auth and *not* for subs; the reason, stop, `userKey` and JWT-derived `user` are carried through while the raw JWT is dropped; an `Authentication Failure` is reported as such (the revoked-credential signal); three closed connections for one NKey come back most-recently-stopped first; the paging envelope passes through; an empty ring is `[]` and not `null`; an unreachable monitor is a 502. `handlers_allowlist_test.go` — the new route is in the BR-040 allowlist. `UsersPanel.spec.js` — the outcome renders for every kind, the stop timestamp only for credentials, nothing renders while the credential has live connections, and an absent row reads as outside the retained window.
- **Not covered:** why a connection closed for any reason the server does not name — the `reason` string is passed through verbatim and never interpreted. The table above is documentation of nats-server's vocabulary, **not** a set the code validates against: nothing rejects or normalises a string that is not in it.

### BR-063 (IMPLEMENTED 2026-09-08) — The refdata notify bridge catches up on the last change per subject, so what it delivers does not depend on start order

`RegisterRefdataNotify` bridges refdata-service's `REFDATA` stream onto `notify._platform.refdata.{context}.{typeKey}.changed`. It was created with **`DeliverNewPolicy`** — "only messages published after this consumer exists".

**Nothing sequences the two services, so that made the bridge's output a race.** `refdata-service` and `shipping-service` start together; neither waits for the other. refdata seeds its corpus and publishes a change per type, while the bridge retries every 5 s because `REFDATA` does not exist yet. Every change published in that window was unreachable **forever** — an ordered consumer created with `New` cannot go back. The symptom was two cells running **identical images off one compose file** reporting different observation counts: au-1 2272, za-1 1136. Nothing in either log explained the difference, because from each service's own point of view nothing had gone wrong.

**The policy is now `DeliverLastPerSubjectPolicy`: one catch-up message per subject.** That is a deliberate middle position, and both neighbours were rejected:

- **`New` (before)** delivers a *subset of history decided by a race*. The one thing a change feed must not be is non-deterministic about what it dropped.
- **`All`** delivers *all* of history. It is deterministic, so it would fix the symptom — but a notification means **"this typeKey is not what you last read, go re-read it"**, and that instruction is discharged in full by delivering it once. Replaying a subject's whole history repeats one instruction N times, and N grows with the stream forever.
- **`LastPerSubject`** is bounded by the size of the **corpus** (contexts × typeKeys) rather than the size of its **history**, and gives the same answer on every start. For a change feed whose payload is an invalidation, last-per-subject *is* the complete truth.

**The catch-up runs once per process, not on every retry.** `nats.go` applies an ordered consumer's deliver policy only to the **initial** request and resumes from its own cursor after a mid-life reset (server restart, consumer deleted underneath it). The bridge's retry loop therefore does not re-replay. This is a property of the library, not of our code, and it is what makes the retry loop and the catch-up safe to combine.

**A restart republishes one notify per known subject, and that burst is correct rather than noisy.** While the service was down it could not know what changed, so "re-read everything you hold" is precisely the true statement. Each republish also emits its BR-045 `obs.pubsub.*` observation, so the burst is doubled on the side-channel — bounded by the corpus, and accepted.

**The bridge logs the size of its own catch-up.** `refdata notify bridge caught up stream=REFDATA republished=N`, emitted when `NumPending` first reaches zero. The bug this rule fixes presented as two numbers that disagreed with no way to attribute either, so the fix ships with the number in the log: a bridge claiming to be deterministic should be able to prove it from one line.

- **Enforced in:** `dictionary/internal/eventhandler/platform_notify.go` — `RegisterRefdataNotify`'s `OrderedConsumerConfig.DeliverPolicy` and the caught-up counter.
- **Test:** `dictionary/internal/eventhandler/platform_notify_test.go`, `Context("BR-063: start order …")` — a change published and acknowledged **before** the bridge starts is still delivered (this spec fails outright under `DeliverNewPolicy`); three changes to one subject plus one to another produce exactly **two** catch-up notifications and no more, carrying the **last** payload per subject (`v3`, not `v1` — which is what separates `LastPerSubject` from `All`); and a change published after the bridge starts is still delivered, guarding the live path. `Consistently` is used alongside `Eventually` on the count, because "exactly two" is the claim and only a settle window can support it.
- **Side effect worth noting:** this package's suite went from ~11.7 s to ~1.4 s. `publishUntilReceived` existed to retry past the `DeliverNew` race; with catch-up it succeeds on the first attempt. A helper written to absorb a race is evidence the race was real.
- **Not covered / deliberately unchanged:** `refdata-service`'s own `refdata/internal/notifybridge` is still `DeliverNewPolicy`. It is **not** the same bug: it runs in-process with the publisher it consumes, so there is no cross-container race, and its own package doc states the losses are intended (`notify.*` is lossy by design; a caller needing the guaranteed feed reads `evt.*` per BR-D42). Also not covered: two `shipping-service` replicas would each catch up and each republish — the notifications are idempotent invalidations so the outcome is correct, but the work is duplicated.
