...
- **`evt.*`** — `Publisher.PublishWithTrace` (`internal/jstream/stream.go`) for shipping and refdata, and `JetStreamEventStore.append` (`organizations-service/.../orchestration/event_store.go`, BR-TP75) for organizations. `PublishWithTrace` has exactly three call sites in the whole backend and all three are `evt.*` publishes — it is not a generic primitive, it is already the `evt.*` choke point, one per service.
...
**Observation is opt-in, and exclusion is expressed by the gate.** Both seams stay silent until observation is enabled — `EnableObservation(nc)` on the `evt.*` side, the `WithObservation(nc)` constructor option on the `notify.*` side. `observability-service`'s two publishers are built without it: `tracestore`'s is that service's internal KV-change plumbing for its own `trace-request-reply` bucket rather than a domain event, and `pubsubstore`'s announces a write to the very bucket these envelopes are stored in, so observing it would be an unbounded feedback loop. Neither needs an entry in a hand-maintained exclusion list any more; "excluded" is now a fact about how the publisher was constructed.
