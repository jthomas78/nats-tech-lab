...
**The error run's span statuses are not the obvious guess, and the rule records them rather than asserting around them.** The root `api.*` span is ERROR, the *caller-side* outbound hop (`refdata.item.get.v1`, the tenant account's local alias) closes **OK** because the request/reply round trip genuinely succeeded — "not found" is a delivered answer, not a transport failure — and refdata's own inbound handler span (`rpc.{context}.refdata.item.get.v1`, the subject the server remapped that alias to) is ERROR, having answered over `browserrpc`'s `respondError` path. So the two ERROR spans sit at the two ends of the domain decision with an OK hop between them. Whether a domain not-found *should* read as ERROR on the callee's own span is a fair question, but settling it means widening `natstrace`'s status vocabulary — it has only OK and ERROR — everywhere, so it is out of scope here.
