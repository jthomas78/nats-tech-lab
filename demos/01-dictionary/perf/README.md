...
## Not in this harness (deferred to Phase 17)

These need architecture that does not exist yet and would be thrown away if
scripted now:

- **Optimistic-concurrency contention** — needs the Phase 14 sequence guard.
- **Cross-stream burst / consumer lag** — needs the Phase 16 `TERMINAL` stream.
- **Cross-aggregate stale-read window** — needs the Phase 16 split.
- **SSE fan-out** — the watch endpoints are streaming, not load-shaped for k6's
  request model; measured in Phase 17.
