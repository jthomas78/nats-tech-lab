...
**PLATFORM's import side is where this rule was corrected** (ADR-047 A1). As originally drafted it mirrored `addPlatformTraceImport` exactly, on the reasoning that "a Stream export needs no per-tenant `LocalSubject` remap: the importing account boundary itself … is what disambiguates which tenant a given imported stream message came from." **That reasoning is wrong, and the rule no longer says it.** The account boundary disambiguates *delivery*, not *provenance*: once N tenants' `obs.pubsub.>` exports are imported onto one identical local subject, a PLATFORM subscriber receives messages with nothing indicating which account they came from. This is exactly why `$SRV.>` and `$JS.API.*` carry `monitorLocalSubjectTmpl` / `jsAPILocalSubjectTmpl` remaps (BR-AC31/BR-AC32), and why the `obs.trace.>` import, which had no remap when this was written, left `TraceWaterfall.vue`'s account gutter able to show only a coarse PLATFORM/TENANT split — the gap BR-AC36 closed in Phase 48.
...
### BR-AC37 (Phase 48d, IMPLEMENTED 2026-08-26) — PLATFORM's cross-account imports converge on every `accounts-service` start, for every account it already knows

BR-AC36 changed a claim that lives in an already-minted JWT, and BR-AC34 changed one before it. Both landed with the same footnote: existing tenants do not get it for free. This rule is the answer to that footnote, stated once so the next export/import pair does not need its own rollout plan.

**`seedPreexistingAccounts` re-runs the full PLATFORM-side import wiring for every seeded tenant, not just for newly-minted ones.** The four `addPlatform*Import` calls that `CreateAccount` makes are reached through one exported `EnsurePlatformImports`, and the startup seed pass calls it per tenant. PLATFORM is skipped: an account must never import itself.

**A healthy stack pushes nothing.** Each of the four calls reads PLATFORM's current claims and returns without a push when the import is already present and correct, so on a converged deployment this pass is four resolver lookups per tenant and zero claims updates. That is the property that makes it safe to run unconditionally at boot rather than behind a flag.

**Boot-time, rather than an operator CLI, deliberately.** The failure this prevents is not "the pass was run wrongly", it is "nobody knew the pass existed" — which is precisely how a stack ends up with a tenant whose imports are two phases behind. A restart is a thing that already happens; a command has to be remembered. The cost is real and accepted: a bug in this path is a bug on every start, and a claims push can now originate from a boot rather than from an explicit act.

**It converges only as far as the underlying calls do.** `addPlatformTraceImport` corrects a wrong `LocalSubject` in place (BR-AC36); `addPlatformPubsubImport` still only dedupes on `(Account, Subject)`. So this rule repairs a stale *trace* remap but would not repair a stale pubsub one. That is not a gap this rule papers over — it is where the next change to that import has to start, and BR-AC36's convergence note is the model for it.

**What it does not do.** It does not touch the tenant-side exports (`tenantExports()` rebuilds those, on a different path), it does not mint or rename accounts, and it does not reach accounts absent from the seed list — an account minted through `POST /api/accounts` got its wiring at creation, and the seed pass is about the day-0 three.

- **Enforced in:** `accounts-service/cmd/main.go` — the `EnsurePlatformImports` call in `seedPreexistingAccounts`; `accounts/provisioner.go` — `EnsurePlatformImports`, the exported wrapper over the four `addPlatform*Import` calls `CreateAccount` already makes.
- **Test:** `accounts/provisioner_test.go` — a pre-BR-AC36 PLATFORM (trace import present, remap absent) is corrected by `EnsurePlatformImports` alone, with no account minted; a second call pushes nothing — asserted by subscribing to `$SYS.REQ.CLAIMS.UPDATE` and requiring silence, **not** by comparing PLATFORM's JWT before and after, since a jti is a content hash of the claims and a gratuitous re-sign of unchanged claims therefore produces a byte-identical token (the first draft of this spec compared IDs and passed against a deliberately broken provisioner); PLATFORM's own public key is refused rather than importing itself. Live verification: BR-051's own, driven across two tenants (Phase 48d).

### BR-AC38 (Phase 50a, IMPLEMENTED 2026-08-27) — A user is recorded before it is signed

**Nothing in a NATS operator-mode stack stores a user.** The resolver holds *account* JWTs only; a user JWT is verified by signature at connect time and never written down server-side; `$SYS.REQ.ACCOUNT.<pub>.CLAIMS.LOOKUP` is account-scoped, so there is no subject that answers "who are this account's users"; and `/connz` knows only who is connected *right now*. This is not a gap in what this service reads — it is the reason the Users panel (Phase 50) cannot exist without this rule. **"List users" is a registry this service writes, not a list it reads.**

**The registry is written by the issuer, at mint time, in this order:** generate the user keypair → write a `pending` row carrying the public NKey → sign the JWT → flip the row to `active` → return the credential. Key generation comes first because an NKey on its own grants nothing — it becomes a credential only when an account signs a JWT for it — so that is the earliest moment the user has an identity to record at all.

**Writing before signing is what makes the failure states legible.** The alternative ordering (sign, then record) has no failure state: a mint that dies between the two simply loses the user, and the panel shows a gap it cannot distinguish from "nothing was minted". Recording first produces a row stuck at `pending`, which is the honest statement of what this service knows — *a credential may have been signed and returned, and its outcome is unknown to the issuer.* That row is **presented as pending, not hidden.**

**Both registry failures are fatal to the mint.** An earlier draft of this rule worried that requiring a registry write would make a Postgres outage stop browser sessions being issued. That reasoning was wrong and does not appear here: every mint path in this service **already** reads Postgres before it signs anything — the account row supplies the signing key seed, and `auth/handler.go` additionally reads the TTL config (BR-AC20) — so the registry adds no dependency that minting did not already have. Issuing a credential that nothing can ever account for is the worse outcome.

**A stuck row is resolved differently by kind** (the `credential` / `session` split, Phase 50 design decision D1):

- A **session** carries an expiry, so a pending session is already dead once its TTL passes and is reaped with no retention grace (`Store.ReapExpiredSessions`, run at start and on the reaper's interval — BR-AC44/AC45; it subsumes the start-up-only `SweepExpiredPendingUsers` this rule originally named). Nothing is lost: the credential it may have issued cannot authenticate any more.
- A **credential** has no expiry (`expires_at IS NULL`) and is therefore **never swept**. It persists as `pending` until an operator resolves it, which is the only safe reading — a `.creds` file that escaped a failed mint is live until it is revoked.

**Deleting a registry row is a registry operation, never a revocation.** A signed user JWT that has escaped is revoked only by adding its NKey to the account's revocation list and pushing the account claims update (`$SYS.REQ.CLAIMS.UPDATE`). Nothing about removing a row from `accounts.users` reaches the server. This is stated as a rule rather than a comment because the Users panel makes deletion look like an action with authority, and it has none.

- **Enforced in:** `accounts/users.go` — the registry types, `UserRegistry`, and `Store.RecordPendingUser` / `MarkUserActive` / `InsertUserIfMissing` / `ListUsers` / `GetUser` / `DeleteUser` / `ReapExpiredSessions`; `accounts/store.go` — the `accounts.users` table (no FK to `accounts.accounts`: `createAccount` mints a tenant's first user *before* it inserts the account row, and a backfilled bootstrap user may name an account this service never recorded); `accounts/provisioner.go` — `CreateUser`'s record → sign → activate order, and `NewProvisioner`'s refusal to construct without a registry (`ErrNoUserRegistry`); `auth/token.go` — the same order in `mintUserToken`, shared by all three `Mint*Token` profiles; `accounts-service/cmd/main.go` — the reaper goroutine that carries the start-of-day sweep (BR-AC45).
- **Test:** `accounts/users_store_test.go` (against a real Postgres) — pending-then-active, a pending row still listed, `ErrUserNotFound` on an unrecorded key, a duplicate NKey refused, and a sweep that takes the expired pending session and leaves the pending credential. `accounts/provisioner_users_test.go` and `auth/token_users_test.go` — the ordering itself, asserted as the recorded call *sequence* against an in-memory registry (`record:<nkey>` then `active:<nkey>`, where the NKey is decoded back out of the credential that was returned), plus: no credential is returned when the pending write fails, nothing is recorded when the failure precedes key generation, an activation failure is reported with the row left pending, and neither mint path can be constructed or called without a registry.
- **Not covered:** revocation. Nothing in this phase adds a revocation path; the rule only fixes what deletion *is not*.

### BR-AC39 (Phase 50a, IMPLEMENTED 2026-08-27) — The registry converges on start for users minted outside this service

`nats/bootstrap-operator.sh` creates `shipping-admin`, `observability`, `sys` and the day-0 tenant credentials with `nsc add user`, before `accounts-service` has ever run. Those users are real and connected, and BR-AC38 never saw them. Without this rule the Users panel would show the sessions the stack minted this morning and none of the credentials it actually runs on.

**The `.creds` file is the only record those users leave**, so the shared creds directory (`NATS_CREDS_DIR`) is read on every start and each decodable file is recorded if it is not already there. This follows BR-AC37's model exactly: **idempotent, no re-sign, no claims update.** Nothing here signs, pushes or rewrites a JWT; a healthy stack on its second start writes zero rows (`InsertUserIfMissing` never overwrites — a converge pass must not rewrite what an earlier run, or a live mint, already recorded).

**A backfilled credential is recorded as `active`, not `pending`.** A `.creds` file that exists on disk was signed successfully; there is no unknown outcome to reconstruct after the fact. It is recorded as `source = bootstrap`, which is what keeps the two provenances distinguishable in the panel — a row this service minted and a row it merely found.

**An issuing account this service has no name for is filed under the issuer's public key rather than dropped.** The account name is resolved from `accounts.accounts` (issuer public key → name), taken from `IssuerAccount` when the JWT was signed by a signing key and from `Issuer` when it was signed by the account identity key, as `nsc` does.

**Warn-and-skip, never fatal.** A missing directory, an unreadable file, or a file that does not decode is logged and skipped: this runs on the start-up path, and a stray file on the creds volume must not be able to stop the service from booting.

- **Enforced in:** `accounts/users_backfill.go` — `BackfillCredsDirUsers`; `accounts-service/cmd/main.go` — `backfillBootstrapUsers`, called after `seedPreexistingAccounts` so account names are already on record when credentials are filed.
- **Test:** `accounts/users_backfill_test.go` — two generated creds files recorded as active bootstrap credentials with the bearer flag carried through; an unknown issuer filed under its public key; an undecodable `.creds` and a non-`.creds` file skipped without failing the pass; a missing directory a no-op. `accounts/users_store_test.go` — `InsertUserIfMissing` writes once and does not rewrite on a re-run.
- **Not covered:** users minted by `nsc` *after* start-up. They are picked up on the next boot, not live — nothing publishes an event when a `.creds` file appears.

### BR-AC40 (Phase 50b, IMPLEMENTED 2026-08-27) — The registry is read over `api._platform.accounts.user.*`, from the PLATFORM account only

The registry BR-AC38 writes is reached through exactly three subjects:

```
api._platform.accounts.user.list.v1    — metadata for every recorded user, cross-tenant
api._platform.accounts.user.get.v1     — one user, with its resolved permissions (BR-AC41)
api._platform.accounts.user.revoke.v1  — revoke one credential (Phase 51b, BR-AC43)
```

**`api.*`, not `rpc.*`.** The Phase 50 design gate provisionally named these `rpc._platform.accounts.user.*`. That naming is wrong and was corrected here: the consumer is the Admin UI's browser connection talking to NATS directly, and **a browser credential is never granted `rpc.>`** (CLAUDE.md, "Subject families"). The subject family follows the *consumer*, not the service that happens to answer — a service-to-service caller would be a second endpoint, not a reason to move these.

**Not REST.** Phases 31–34 made browser business paths NATS-only; the Phase 33 refdata REST exemption is scoped to refdata and does not extend to a new surface. This is also why the endpoints carry an explicit `.v1` and a fixed-literal `_platform` context token rather than a wildcard — a subject is a public interface, and the frontend's permission grant matches it exactly.

**The account boundary is the authorization.** The adapter is mounted on the PLATFORM connection *only* (`cmd/main.go` registers it under `if platformNC != nil`), so a tenant connection cannot reach these subjects at all — there is no subject to publish to, rather than a handler that checks and refuses. The reciprocal grant on the browser side **enumerates** the subjects, never an `api._platform.accounts.>` prefix: the allowlist is exact precisely so a future accounts endpoint is not reachable from a browser merely by being named consistently with these.

**Phase 51b added a write to this surface.** `.revoke.v1` is a mutation on an `api.*` subject, and it is guarded by nothing in the handler — the PLATFORM-only mount above is the whole guard, and it is the same guard the reads already rely on. What did change is that `MintAdminToken` builds its grant from `UsersAdapterSubjects()`, so registering an endpoint now grants it to the Admin UI as a side effect. `auth/token_test.go`'s exact `ConsistOf` is the deliberate countermeasure: adding an endpoint breaks that assertion, and the fix is to decide about the subject rather than to paste it in.

**The list is metadata only, and the registry never returns a secret.** `UserSummary` carries the public NKey, name, account, kind, status, bearer flag, source and timestamps — no seed, no `.creds`, no signed JWT. This is a property of the *type*, not of the handler: BR-AC38's registry stores claims and never the signed token, because a bearer JWT is a credential on its own and an endpoint that returned one would be handing out access rather than describing it.

**The list is deliberately cross-tenant.** It is a platform-operator view: a Users panel that could not show every account's users could not answer the question it exists for.

- **Enforced in:** `accounts/usersrpc.go` — `UserListSubject` / `UserGetSubject`, `NewUsersAdapter`, the `endpoints()` registration table (the single source of truth both the service and the permission tests read) and `UsersAdapterSubjects()`; `accounts-service/cmd/main.go` — the PLATFORM-only mount; `auth/token.go` — `MintAdminToken` adding `accounts.UsersAdapterSubjects()` to `Permissions.Pub.Allow`, one subject at a time.
- **Test:** `accounts/usersrpc_test.go` (end-to-end over an embedded NATS server) — exactly the two subjects are registered, every one is `api._platform.accounts.`-prefixed and wildcard-free, `.list.v1` returns metadata rows, the marshalled list contains no `seed` / `creds` / `jwt` substring, the list spans tenants, `.get.v1` returns the claims view, an unknown key answers `notFound`, and an empty body is an error. `auth/token_test.go` — the PLATFORM credential's `Pub.Allow` is the exact six-subject set, still with no `api.>`.
- **Not covered:** creation and deletion. `.revoke.v1` (BR-AC43) is the only mutation; nothing here mints a user or removes a registry row over NATS.

### BR-AC41 (Phase 50b, IMPLEMENTED 2026-08-27) — A user's permissions are shown as resolved, not as signed

**When an account signs a user with a *scoped* signing key, the server discards the user JWT's own permissions and applies the key's template instead.** Rendering the JWT verbatim would therefore show access the user does not have — and, worse, would make a wrongly-minted credential look identical to a correct one, because the grants that were overridden are exactly the ones that would be wrong. So the registry resolves before it reports.

**Resolution reads the signing key, which is why the row records it.** `IssuerKey` is the key that actually *signed* — not derivable from `AccountKey`, since a signing key is a different key from the account identity — and it is the lookup key for the scope. Resolution asks the account's claims for `SigningKeys.GetScope(IssuerKey)` and takes:

- **a scoped signing key** → the scope's `Template` is authoritative and becomes `Effective`; the user's own grants are still returned as `JWTGrants`, which the panel shows struck through beneath (Phase 50 design decision D4). Effective-only was rejected: an operator debugging a mint needs to see what was *asked for* as well as what was granted, and the difference between the two is the bug.
- **a plain (unscoped) signing key, or the account identity key** → the user's own grants are effective; `JWTGrants` stays empty, because nothing was discarded and showing an identical struck-through copy would imply an override that did not happen.

**An unresolvable scope is a stated reason, not an error.** If the account cannot be looked up, the row has no issuer key, or the issuer is not a signing key on the account any more (rotated since the user was minted), the view returns normally with `Unresolved` set and `Scoped` false. The row is still shown — hiding a user because its account rotated would be a worse answer than showing it — but its grants are never labelled "effective" when they might not be.

**No recorded permissions is distinct from empty permissions.** A `nil` `Effective` means *this service has no record of what this user was granted* and is reported as an `Unresolved` reason. It is not flattened to an empty `jwt.UserPermissionLimits`, which marshals as "allowed nothing" — a far stronger and quite different claim. This is also why `marshalPermissions` writes SQL `NULL` rather than the JSON literal `null`.

- **Enforced in:** `accounts/userclaims.go` — `UserClaimsReader.Get` / `List`, `resolveScope`, and the `AccountClaimsLookup` / `UserReadStore` ports that keep the whole resolution testable without NATS or Postgres (`*Provisioner` and `*Store` satisfy them in `cmd/main.go`); `accounts/users.go` — the `IssuerKey` and `Permissions` columns, `marshalPermissions` / `finishUser`, and the decision to store the *claims* and never the signed token; `accounts/store.go` — `Migrate`'s two `ADD COLUMN IF NOT EXISTS` statements, deliberately ALTER-not-CREATE so a stack already carrying a 50a `accounts.users` table converges without a reseed; `accounts/provisioner.go` and `auth/token.go` — building the claims *before* the pending row so the permissions are known when it is written; `accounts/users_backfill.go` — capturing an nsc-minted user's grants on the one pass that ever sees its `.creds`.
- **Test:** `accounts/userclaims_test.go` — a scoped signing key makes the template effective (and `api.>` from the user's own broader grants is asserted *absent* from `Effective`) with `JWTGrants` populated and the scope role carried through; a plain signing key and the account identity key both leave the user's own grants effective with `JWTGrants` empty; a lookup error, a missing issuer key and an issuer that is not a signing key each set `Unresolved` without returning an error; no recorded permissions yields nil `Effective` plus a reason; `List` is metadata only; an unknown key is `ErrUserNotFound`.
- **Not covered:** account-level limits and connection-type restrictions beyond what `jwt.UserPermissionLimits` carries, and any check that the *server* agrees — resolution mirrors the server's documented precedence rather than observing it.

### BR-AC42 (Phase 51b, IMPLEMENTED 2026-08-27) — A revocation is an amendment to the account JWT; the registry only mirrors it

**Nothing in NATS stores a user, so nothing in NATS can delete one.** A user JWT is verified by signature and discarded; the server never held a copy to remove. The one place a *specific* user can be denied is the issuing **account** JWT's revocation list, which names user public keys and the time from which they are refused. Revoking therefore means: look the account's current claims up over `$SYS.REQ.ACCOUNT.<pub>.CLAIMS.LOOKUP`, add the entry, re-sign with the **operator** signing key — it is the account JWT being re-signed, not a user JWT — and push `$SYS.REQ.CLAIMS.UPDATE`.

**The claims are amended in place, never rebuilt.** `RevokeUser` mutates the looked-up `AccountClaims` and re-encodes it. Rebuilding the account from `newAccountClaims` would be a silent revert of every limit, export, import or signing key added since it was created — a revocation that quietly undoes unrelated provisioning is worse than no revocation.

**`revoked_at`, not `status = 'revoked'`.** `Status` already carries a different axis — whether the mint completed — and a revoked row keeps that history: a credential that was *active* and is now revoked is not the same thing as one whose mint never finished. A timestamp also answers "when", which a status value cannot.

**The registry is the mirror and the JWT is the authority, so the push goes first.** `UserRevoker.Revoke` pushes the claims update and stamps the row only once the server has accepted it. The other ordering fails the dangerous way round — a row marked revoked while the credential still works tells an operator a credential is dead when it is live, which is the exact failure this capability exists to prevent. This way round the worst case is a live credential correctly refused by the server and a registry that has not caught up, which is visible drift and recoverable; the error message says explicitly which half landed, because the remedy is to reconcile the row and *not* to revoke again.

**A second revocation is refused rather than re-stamped.** Checked before the push, so pressing the button twice does not re-sign and redistribute an account JWT across the deployment for a credential that is already dead, and does not move a timestamp that recorded when access actually stopped.

**No `revoked_by`.** There is no operator identity in this POC — the Admin UI connects as the shared `admin-app` credential — so the column would record the same worthless string every time while implying an audit trail that does not exist.

- **Enforced in:** `accounts/provisioner.go` — `Provisioner.RevokeUser` (lookup → `Revocations.Revoke` → encode with the operator signing key → `pushClaimsUpdate`); `accounts/users.go` — the `revoked_at` column, `RevokedAt`, `MarkUserRevoked` and `ErrUserAlreadyRevoked`; `accounts/store.go` — `Migrate`'s `ADD COLUMN IF NOT EXISTS revoked_at`, ALTER-not-CREATE so an existing stack converges without a reseed; `accounts/usersrevoke.go` — `UserRevoker.Revoke`'s ordering; `accounts/userclaims.go` — `RevokedAt` on `UserSummary`.
- **Test:** `accounts/usersrevoke_test.go` — the row is stamped only after the push is accepted (a failed push leaves `revoked_at` NULL and pushes nothing), a failed stamp after a successful push reports which half landed, a second revocation is `ErrUserAlreadyRevoked` and pushes nothing, and a revoker with no NATS path refuses outright. `accounts/users_store_test.go` — `MarkUserRevoked` stamps `revoked_at` while leaving `status` alone, refuses a second call rather than moving the timestamp, and reports an unknown key as `ErrUserNotFound`.
- **Not covered:** whether the running server actually refuses the credential afterwards. That is the live-stack verification, not a spec — `jwt` v2.8.2's `IsRevoked` compares `ts >= iat`, so there is no same-second escape gap, but this suite never asks a server.

### BR-AC43 (Phase 51b, IMPLEMENTED 2026-08-27) — Only a long-lived credential can be revoked, and revocation is terminal

**Credentials only.** A revocation aimed at a `session` row is refused (`ErrRevokeNotCredential`). A browser session is a 15-minute ephemeral mint that dies on its own TTL faster than an operator could find its row, and offering to revoke one would spend an account-JWT re-sign and a cluster-wide push on something already expiring.

**A row with no recorded issuing account is refused** (`ErrRevokeAccountUnknown`). Revocation is an amendment to a *specific* account's JWT; without one there is nothing to amend, and guessing would either be a no-op or write the revocation into the wrong account.

**The public key is the identity.** `.revoke.v1` takes `publicKey` and nothing else — a name is not unique enough to revoke by (BR-058), and two users may share one.

**Revocation is terminal — there is no un-revoke.** `jwt.RevocationList.ClearRevocation` exists and would reuse this exact path; it is deliberately not surfaced. Recovery from a mis-revocation is minting a **replacement** credential. Two consequences are accepted knowingly:

- Revoking a service credential by mistake takes that service down until a new one is minted and its `.creds` mounted.
- The entry is permanent in practice. `RevocationList.MaybeCompact` prunes only entries superseded by a `*` / `jwt.All` wildcard — it does **not** prune by expiry — so an account JWT accumulates one entry per revoked credential for the life of the account.

The reversible lifecycle that does exist is the tenant one (suspend / reactivate, BR-AC10–AC12), which this does not replace and is the right tool when the whole account, not one credential, is the problem.

**A revoked credential shows as revoked in the panel, and that state outranks the others.** The Users panel's `revoked` health state takes precedence over `pending` and over expiry, because a revoked credential's other attributes have stopped mattering. This reads the registry row, not a connection, so it does not reintroduce the "liveness is not identity" mistake BR-060 exists to prevent.

- **Enforced in:** `accounts/usersrevoke.go` — `UserRevoker.Revoke`'s kind, already-revoked and account-key checks, and the file header recording what is deliberately not exposed; `accounts/usersrpc.go` — `UserRevokeSubject`, `UserRevokeRequest` and `handleRevoke`; `accounts-service/cmd/main.go` — the revoker wired from the same `Provisioner` that holds the operator signing key; `frontend/admin/src/components/UsersPanel.vue` — the Revoke control offered for unrevoked `credential` rows only, behind a confirmation naming the credential and its live connection count.
- **Test:** `accounts/usersrevoke_test.go` — a credential is revoked against its own account key; a session, a row with no account key and an empty public key are each refused with nothing pushed; an unknown user is `ErrUserNotFound`; a second revocation is refused and the stamped row cannot be walked back through this path. `accounts/usersrpc_test.go` — `.revoke.v1` end-to-end over an embedded server, returning the stamped row and carrying the session refusal out to the caller, with no seed / creds body / signed JWT in the reply. `frontend/admin/src/components/UsersPanel.spec.js` — the control is absent for sessions and for already-revoked rows, the confirmation names the credential, and `revoked` outranks the other health states.
- **Not covered:** un-revocation, by design; and bulk revocation — one call revokes one credential.

### BR-AC44 (Phase 52, IMPLEMENTED 2026-08-27) — An expired session row is reaped once it is older than the retention window; credentials and revoked rows never are

**The registry grows on every browser connect and nothing used to shrink it.** BR-AC38 makes recording part of minting, so a 15-minute session (BR-AC20) writes a row that outlives the credential it describes by forever. The only sweep that existed ran once at start-up and touched `status = 'pending'` rows — the mint-crash case, not the ordinary one. Measured on the running stack after roughly a day's uptime: **56 rows, 44 of them expired sessions**.

**Reaping is a registry operation, never a revocation** — the same line BR-AC42 draws for `DeleteUser`. Removing a row does not reach the server and does not stop any credential working. It is safe here only because the rows it removes describe credentials that are *already dead on their own TTL*.

**What may be deleted is a whitelist, not an exclusion list.** Every clause narrows the predicate, so the failure mode of a future edit is a row that survives rather than a credential that does not:

- `kind = 'session'`. A credential has no expiry and cannot match today; the clause is belt-and-braces, and it means a bug that stamps `expires_at` onto a credential costs a wrong badge in the panel rather than a deleted credential.
- `expires_at IS NOT NULL` **and already in the past**. The window is measured from the row's own expiry, never from `now()` alone — so even a retention of `0` cannot touch a live session.
- `revoked_at IS NULL`. **A revoked row is never reaped, at any age.** The row is the only human-readable mirror of an entry in the account JWT's revocation list (BR-AC42/BR-AC43); that list keeps the NKey forever and `MaybeCompact` does not prune by expiry, so deleting the row leaves an operator staring at a revocation whose subject nothing in the stack can name. Revocations are rare and operator-driven, so exempting them does not reintroduce unbounded growth.

**Retention and cadence are different questions and are two separate knobs.** Conflating them is how a reaper ends up either useless or destructive:

- **Retention** (`ACCOUNTS_SESSION_RETENTION`, default `24h`) is how long a row survives *past its own `expires_at`*. It is the operator-facing knob. A session that expired four minutes ago is exactly the row someone is reading when they ask "why did my tab drop", so reaping on expiry destroys the answer at the moment it is wanted. 24h is chosen because it is roughly how long the row stays *explainable*: past that, the `/connz` closed ring feeding the panel's **Last outcome** column (bounded, measured at ~59 entries) has long since rolled over, and a row with no outcome left to pair with is not worth keeping.
- **Cadence** (`ACCOUNTS_REAPER_INTERVAL`, default `15m`) is a load knob and nothing else — see BR-AC45.

**A pending row is reaped the instant its TTL passes, with no retention grace.** This is the behaviour BR-AC38 already had and Phase 52 does not change it. The window exists to keep an *explainable* row around long enough to be read; a mint that never produced a credential has nothing to explain.

**An explicit retention of `0` disables the reaper; it does not mean reap-on-expiry.** Both readings are defensible in the abstract and only one of them is safe to get wrong. An operator who types the obvious value and means "don't keep them" gets a visible no-op in the logs, not a table emptied of everything that could have explained the last hour. A negative value is rejected outright rather than clamped — it can only be a mistake, and clamping would hide which mistake.

**The panel's `hide expired` checkbox is not a substitute and its default moved because of this rule.** It shipped defaulted ON earlier the same day, when the expired set was the stack's whole uptime. With the reaper bounding that set at one retention window, the default is now OFF: a session that died inside the last day is the row an operator wants, and hiding it by default would hide the useful part. The checkbox stays, because a window is a policy and 24h is not everyone's.

- **Enforced in:** `accounts/users.go` — `ReapExpiredSessions`, the whitelist predicate, and the header comment recording why each clause is there; `accounts/reaper.go` — `ReaperConfig`, `ReaperConfigFromEnv` and the two defaults; `accounts-service/cmd/main.go` — the config parsed at boot, fatal on a bad value; `deploy/global/compose.control.yaml` — both variables shown at their defaults so they are discoverable there and not only in Go; `frontend/admin/src/components/UsersPanel.vue` — `hideExpired` now defaulting to `false`.
- **Test:** `accounts/users_store_test.go` (BR-AC44) — a session that expired inside the window survives and one outside it is removed; a credential carrying a long-past expiry is never reaped; a session with no expiry is never reaped; a revoked session 90 days past expiry is never reaped and keeps its `revoked_at`; the removed count is reported; and a retention of `0` still refuses to touch an unexpired session. `accounts/reaper_test.go` — the configuration rules, including explicit-zero-is-off and reject-rather-than-fall-back. `frontend/admin/src/components/UsersPanel.spec.js` — expired rows visible by default, hidden on request with a held-back count, and not double-counted against both chips.
- **Not covered:** reaping any other table; a retention policy that varies by account or by kind; and any archival step — a reaped row is gone, not moved.

### BR-AC45 (Phase 52, IMPLEMENTED 2026-08-27) — The reaper runs at start-up and then on a fixed interval, and a failed run never stops it

**It reaps once immediately, before the first tick.** Not an optimisation: a stack that has been down for a week must not wait out an interval before cleaning up, and the start-up run is also what surfaces a misconfigured retention now rather than a quarter of an hour from now.

**The interval is a load knob only.** It has to be frequent enough that between-sweep accumulation is small next to the retained set, and nothing more. At the default `15m` — one minimum-TTL period (`MinTTLMinutes`) — against a `24h` window that is roughly 1% overshoot. Hourly would be equally defensible; anything under about five minutes is churn against an empty predicate.

**A failing tick logs and waits for the next one.** A reaper that returns on the first Postgres blip has silently stopped reaping, which is worse than one that is noisily failing: the table grows either way, but only one of them says so. The reaper likewise never stops the service — same warn-and-continue posture as `backfillBootstrapUsers`.

**A bad *configuration*, by contrast, is fatal at boot.** An unparseable or negative duration is not a degraded reaper, it is a deployment that meant something it did not say. A deployment that meant to keep a week of sessions and mistyped it must find out at start-up, not a week later when the rows it wanted are already gone.

**It is silent when it removes nothing.** A heartbeat every fifteen minutes saying `0` is how a log stops being read.
...
**The writable directory is per-project, so it is per-cell.** `poc` and `lb-au-1` do not share minted tenants, the same way they do not share a Postgres. Only the seeded accounts are common to both.
...
- **Verified live:** minting `credstest` in `poc` wrote `credstest.creds` (`0600 app:app`) into the volume, `shipping-service` saw it through its read-only mount of the same volume, `git status` on `nats/creds/` stayed clean, and suspending the tenant removed the file again.
