...
- A **session** carries an expiry, so a pending session is already dead once its TTL passes and is swept (`Store.SweepExpiredPendingUsers`, run on start). Nothing is lost: the credential it may have issued cannot authenticate any more.
...
The registry BR-AC38 writes is read back through exactly two subjects:
...
**The account boundary is the authorization.** The adapter is mounted on the PLATFORM connection *only* (`cmd/main.go` registers it under `if platformNC != nil`), so a tenant connection cannot reach these subjects at all — there is no subject to publish to, rather than a handler that checks and refuses. The reciprocal grant on the browser side lists the two subjects **individually**, never an `api._platform.accounts.>` prefix: the allowlist is exact precisely so a future accounts endpoint is not reachable from a browser merely by being named consistently with these two.
